...
function UserApp({ user }) {
  ...
  return (
    <div>
      <div style={{
        background: "rgba(255,255,255,0.9)", backdropFilter: "blur(10px)", color: "#064E3B", padding: "0 24px",
        display: "flex", alignItems: "center", justifyContent: "space-between",
        minHeight: 48, fontFamily: "'Poppins', system-ui, -apple-system, 'Segoe UI', sans-serif",
        borderBottom: "1px solid #D6DCE5", boxShadow: "0 2px 14px rgba(6, 78, 59,0.06)",
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 0 }}>
          <Wordmark size={30} />
          <span style={{ color: "#D6DCE5", fontSize: 16 }}>/</span>
          <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", gap: 1, minWidth: 0 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: "#3D5A50", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {companyName || user.companyCode}
            </span>
            <span style={{ fontSize: 10, color: "rgba(6, 78, 59,0.4)", letterSpacing: "0.3px" }}>
              <DevCredit style={{ color: "rgba(6, 78, 59,0.4)" }} />
            </span>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <span style={{ fontSize: 13, color: "#3D5A50", fontWeight: 600 }}>👤 {user.username}</span>
          <button
            onClick={handleLogout}
            style={{ background: "#fff", color: "#df5346", border: "1.5px solid #f3c6c1", borderRadius: 6, padding: "5px 14px", cursor: "pointer", fontSize: 12, fontWeight: 700 }}
          >
            Logout
          </button>
        </div>
      ...
      {loadingId && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(255,255,255,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, fontFamily: "sans-serif", fontSize: 18, color: "#064E3B" }}>
