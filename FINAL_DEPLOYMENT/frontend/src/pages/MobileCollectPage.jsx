  ...
  if (submitted) return (
    <div style={S.page}>
      ...
      <div style={{ ...S.card, textAlign: "center" }}>
        ...
        <h2 style={{ color: !isOnline ? "#064E3B" : "#0E9E6E", margin: "0 0 8px" }}>{!isOnline ? "Saved Offline" : "Submitted!"}</h2>
    ...
    <div style={S.page}>
      ...
      <div style={S.header}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.55)", letterSpacing: "0.14em", textTransform: "uppercase", marginBottom: 3 }}>Mulyanka</div>
      ...
      <form onSubmit={handleSubmit} style={S.form}>
                ...
                {[["radp","Ro-Aa-Pa-Dam"],["bkd","Bi-Ka-Dhur"]].map(([val, lbl]) => (
                  <button key={val} type="button" onClick={() => setAreaUnit(val)}
                    style={{ padding: "4px 10px", fontSize: 11, fontWeight: 600, cursor: "pointer", border: "none",
                      background: areaUnit === val ? "#10B981" : "#f5f5f5",
        ...
        <Section icon="🏗️" title="Building">
            ...
            {[
              ...
              <button key={val} type="button"
                ...
                style={{ padding: "10px 14px", borderRadius: 8, textAlign: "left",
                  border: `2px solid ${hasBuilding === val ? "#10B981" : "#ddd"}`,
                  background: hasBuilding === val ? "#E8F5EC" : "#F8FAF9",
                  color: hasBuilding === val ? "#10B981" : "#555",
          ...
          {hasBuilding === "yes" && (
            <div>
              ...
              <div style={{ marginTop: 18, borderTop: "1.5px solid #D6DCE5", paddingTop: 14 }}>
                <div style={{ fontWeight: 700, color: "#064E3B", fontSize: 13, marginBottom: 12, letterSpacing: 0.2 }}>📐 Technical Specifications</div>
            ...
            {HAZARD_LIST.map(({ key, label, hasSide, hasDist }) => (
              <div key={key} style={{ border: `1.5px solid ${hazards[key] ? "#f0c070" : "#e5e7eb"}`, borderRadius: 8, padding: "10px 12px", background: hazards[key] ? "#fffaf4" : "#fff" }}>
                <button type="button" onClick={() => toggleHazard(key)}
                  ...
                  <span style={{ width:20, height:20, borderRadius:4, border:`1.5px solid ${hazards[key]?"#D4AF37":"#ccc"}`, background: hazards[key]?"#fff3d6":"#f5f5f5", display:"inline-flex", alignItems:"center", justifyContent:"center", fontSize:13, color:"#D4AF37", fontWeight:700, flexShrink:0 }}>{hazards[key] ? "✓" : ""}</span>
        ...
        <Section icon="📋" title="Missing Documents Checklist">
          ...
          {(() => {
            ...
            const sections = [
              { label: isBuySell ? "Buyer / Seller" : "Person", color: "#10B981", docs: personDocs },
              { label: "Company", color: "#b45309", docs: companyDocs },
              { label: "Land", color: "#0E9E6E", docs: landDocs },
          ...
          {(() => {
            ...
            if (missing.length === 0) return <div style={{ marginTop: 14, padding: "10px 14px", borderRadius: 8, background: "#E8F5EC", border: "1.5px solid #10B981", color: "#0E9E6E", fontWeight: 700, fontSize: 13 }}>✅ All documents available</div>;
            return (
        ...
        <div style={{ padding: "0 0 20px" }}>
          <button type="submit" style={{ ...S.btnSubmit, background: isOnline ? "#D4AF37" : "#064E3B" }} disabled={submitting || compressing}>
          ...
          <button type="button" style={{ width:"100%", padding:"13px", background:"#fff", color:"#064E3B", border:"2px solid #064E3B", borderRadius:10, fontSize:15, fontWeight:700, cursor:"pointer" }}
            onClick={async () => {
              const payload = buildPayload();
...
function OfflineBanner({ pendingCount }) {
  return (
    <div style={{ background:"#04382A", color:"#fff", padding:"10px 20px", display:"flex", alignItems:"center", gap:10, fontSize:13 }}>
...
function DocCheckRow({ label, checked, onToggle }) {
  return (
    <button type="button" onClick={onToggle}
      style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", border: `1.5px solid ${checked ? "#10B981" : "#ddd"}`, borderRadius: 8, padding: "10px 12px", background: checked ? "#E8F5EC" : "#fff", cursor: "pointer", marginBottom: 7, textAlign: "left" }}>
      <span style={{ width: 20, height: 20, borderRadius: 4, border: `1.5px solid ${checked ? "#10B981" : "#ccc"}`, background: checked ? "#10B981" : "#f5f5f5", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 13, color: "#fff", fontWeight: 700, flexShrink: 0 }}>
        {checked ? "✓" : ""}
      </span>
      <span style={{ fontSize: 14, fontWeight: checked ? 600 : 400, color: checked ? "#0E9E6E" : "#444", flex: 1 }}>{label}</span>
      <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 10, background: checked ? "#10B981" : "#f39c12", color: "#fff", whiteSpace: "nowrap", flexShrink: 0 }}>
...
function Section({ icon, title, children }) {
  return (
    <div style={{ marginBottom: 8 }}>
      <div style={{ background: "#064E3B", padding: "10px 20px", display:"flex", alignItems:"center", gap:8 }}>
...
const S = {
  page:    { minHeight:"100vh", background:"#F3F6F5", fontFamily:"'Poppins', system-ui, -apple-system, 'Segoe UI', sans-serif" },
  header:  { background:"#064E3B", padding:"16px 20px", display:"flex", alignItems:"center", gap:12 },
  ...
  btnGps:  { padding:"11px 13px", background:"#064E3B", color:"#fff", border:"none", borderRadius:8, cursor:"pointer", fontWeight:600, whiteSpace:"nowrap", fontSize:14 },
  ...
  btnSecondary:{ marginTop:16, padding:"12px 28px", background:"#064E3B", color:"#fff", border:"none", borderRadius:8, fontSize:15, cursor:"pointer", fontWeight:600 },
  btnAdd:  { padding:"7px 13px", background:"#E8F5EC", color:"#064E3B", border:"1px solid #BFE3C9", borderRadius:7, cursor:"pointer", fontSize:13, fontWeight:600 },
  ...
  groupTitle:{ fontSize:12, fontWeight:700, color:"#064E3B", marginBottom:10 },
  ...
  pendingBadge:{ background:"#E8F5EC", border:"1px solid #BFE3C9", borderRadius:8, padding:"10px 16px", margin:"12px 0", color:"#064E3B", fontSize:13, fontWeight:600 },
  syncToast:{ position:"fixed", top:16, left:"50%", transform:"translateX(-50%)", background:"#0E9E6E", color:"#fff", padding:"12px 24px", borderRadius:10, fontSize:14, fontWeight:700, zIndex:9999, boxShadow:"0 4px 16px rgba(0,0,0,0.2)", whiteSpace:"nowrap" },
};
