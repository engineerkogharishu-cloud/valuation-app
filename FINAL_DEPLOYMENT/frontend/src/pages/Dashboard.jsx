...
function StatCard({ icon, label, value, sub, color }) {
  return (
    <div style={{ ...S.statCard, borderTop: `3px solid ${color}` }}>
      ...
      <div style={{ fontSize: 28, fontWeight: 700, color: "#064E3B", lineHeight: 1 }}>{value}</div>
    ...
    <div style={S.page}>
      ...
      <div style={S.hero}>
        <div style={S.heroInner}>
          <div>
            <Wordmark size={26} style={{ marginBottom: 12 }} />
      ...
      <div style={S.content}>
        ...
        <div style={S.topRow}>
          <div style={S.statsGrid}>
            <StatCard icon="📋" label="Total Reports" value={total} sub="all time" color="#10B981" />
            <StatCard icon="📝" label="Preliminary" value={preliminary} sub="draft reports" color="#D4AF37" />
            <StatCard icon="🧾" label="Bill" value={bills} sub="bill reports" color="#D4AF37" />
            <StatCard icon="✅" label="Final" value={final} sub="completed reports" color="#10B981" />
          </div>
                    ...
                    <tbody>
                      {reports.map((r, i) => (
                        <tr key={r.id} style={S.tr}
                          ...
                          <td style={{ ...S.td, fontWeight: 600, color: "#064E3B" }}>{r.client_name || "—"}</td>
                          <td style={S.td}>{r.bank || "—"}</td>
                          <td style={S.td}>{r.branch || "—"}</td>
                          <td style={S.td}>
                            <span style={{
                              ...S.badge,
                              background: r.report_type === "preliminary" ? "#fff3e0" : r.report_type === "bill" ? "#fff8ee" : "#E8F5EC",
                              color:      r.report_type === "preliminary" ? "#e65100" : r.report_type === "bill" ? "#D4AF37" : "#0E9E6E",
                              border: `1px solid ${r.report_type === "preliminary" ? "#ffcc80" : r.report_type === "bill" ? "#f6d08a" : "#A7E3B1"}`,
                            }}>
        ...
        {tab === "final" && (
          <ReportTypeTab
            ...
            badgeBg="#E8F5EC" badgeColor="#0E9E6E" badgeBorder="#A7E3B1" badgeText="✅ Final"
            hoverBg="#E8F5EC"
            onOpen={onOpen} onExport={handleExport} onDelete={id => setDelId(id)} user={user}
          />
        )}
        ...
        {tab === "bill" && (
          <ReportTypeTab
            ...
            badgeBg="#fff8ee" badgeColor="#D4AF37" badgeBorder="#f6d08a" badgeText="🧾 Bill"
            hoverBg="#fffbf0"
            showBillNo
            onOpen={onOpen} onExport={handleExport} onDelete={id => setDelId(id)} user={user}
          />
      ...
      {delId && (
        <div style={S.overlay}>
          <div style={S.modal}>
            ...
            <h3 style={{ textAlign: "center", margin: "0 0 8px", color: "#064E3B" }}>Delete Report #{delId}?</h3>
...
const S = {
  page: { minHeight: "100vh", background: "linear-gradient(180deg,#F8FAF9 0%,#F3F6F5 260px,#F3F6F5 100%)", fontFamily: "'Poppins', system-ui, -apple-system, 'Poppins', system-ui, -apple-system, 'Segoe UI', sans-serif" },

  // Hero
  hero: { background: "transparent", padding: "34px 0 24px", borderBottom: "1px solid #D6DCE5" },
  ...
  greeting: { color: "#7A8F87", fontSize: 14, margin: "0 0 4px" },
  heroTitle: { color: "#064E3B", fontSize: 28, fontWeight: 800, margin: 0, letterSpacing: "-0.5px" },
  heroSub: { color: "#7A8F87", fontSize: 13, margin: "6px 0 0" },
  heroCta: { background: "linear-gradient(135deg,#D4AF37,#D4AF37)", color: "#fff", border: "none", borderRadius: 10, padding: "12px 24px", fontWeight: 700, fontSize: 15, cursor: "pointer", whiteSpace: "nowrap", boxShadow: "0 6px 16px rgba(212, 175, 55,0.32)" },
  ...
  creditBalance: { fontSize: 48, fontWeight: 800, color: "#064E3B", lineHeight: 1, letterSpacing: "-1px" },
  ...
  tabBar: { display: "flex", background: "#EDF2EF", borderRadius: 10, padding: 4, gap: 4 },
  ...
  tabActive: { background: "#fff", color: "#064E3B", boxShadow: "0 1px 6px rgba(0,0,0,0.12)" },

  // Buttons
  btnPrimary: { background: "#D4AF37", color: "#fff", border: "none", borderRadius: 8, padding: "11px 22px", fontWeight: 700, cursor: "pointer", fontSize: 14 },
  btnOutline: { background: "#fff", color: "#064E3B", border: "1.5px solid #C8D6CD", borderRadius: 8, padding: "8px 16px", fontWeight: 600, cursor: "pointer", fontSize: 13 },
  ...
  btnSearch: { padding: "10px 20px", background: "#064E3B", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 600, fontSize: 13 },
  ...
  th: { background: "#F3F6F5", color: "#7A8F87", padding: "13px 16px", textAlign: "left", fontWeight: 700, whiteSpace: "nowrap", fontSize: 11, letterSpacing: "0.4px", textTransform: "uppercase", borderBottom: "1px solid #D6DCE5" },
  tr: { transition: "background 0.1s", cursor: "default" },
  td: { padding: "13px 16px", color: "#333", borderBottom: "1px solid #F3F6F5" },
  tdActions: { padding: "10px 16px", whiteSpace: "nowrap", borderBottom: "1px solid #F3F6F5" },
  badge: { padding: "3px 10px", borderRadius: 20, fontSize: 11.5, fontWeight: 700 },
  tableFooter: { padding: "12px 18px", borderTop: "1px solid #F3F6F5", fontSize: 12, color: "#aaa", textAlign: "right" },

  // Action buttons in table
  btnOpen: { background: "#064E3B", color: "#fff", border: "none", borderRadius: 6, padding: "6px 14px", cursor: "pointer", marginRight: 6, fontSize: 12, fontWeight: 600 },
...
function ReportTypeTab({ reports, icon, label, emptyIcon, emptyMsg, badgeBg, badgeColor, badgeBorder, hoverBg, showBillNo, onOpen, onExport, onDelete, user }) {
  ...
  return (
    <div style={S.tableCard}>
      <div style={{ padding: "14px 18px", borderBottom: "1px solid #F3F6F5" }}>
        <span style={{ fontWeight: 700, fontSize: 15, color: "#064E3B" }}>{icon} {label}</span>
