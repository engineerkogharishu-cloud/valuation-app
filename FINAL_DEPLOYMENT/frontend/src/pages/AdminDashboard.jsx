...
import * as BRAND_TOKENS from "../constants/brand";
...
// Palette, gradients, tonal sets and type scale all come from the Mulyanka
// brand tokens so a change there repaints the whole app.
const { C, GRAD, TONE, FONT_DISPLAY, FONT_MONO, FONT_UI } = BRAND_TOKENS;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
          <h3 style={{ margin: 0, color: C.navy, fontSize: 18, fontWeight: 700, fontFamily: FONT_DISPLAY, letterSpacing: "-0.01em" }}>{title}</h3>
          <button onClick={onClose} style={{
            width: 30, height: 30, border: "none", borderRadius: 8,
            background: "#F3F6F5", cursor: "pointer", fontSize: 15,
...
function ReportMapSection() {
  ...
  useEffect(() => {
    ...
    for (const cluster of points) {
      ...
      const bg    = count > 9 ? "#c0392b" : count > 4 ? "#e67e22" : "#10B981";
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 1px 2px rgba(6, 78, 59,.05), 0 6px 20px -12px rgba(6, 78, 59,.18)", overflow: "hidden", minHeight: 520, display: "flex", flexDirection: "column" }}>
      {/* Header */}
      <div style={{ padding: "16px 22px", borderBottom: "1.5px solid #D6DCE5", display: "flex", alignItems: "center", justifyContent: "space-between", background: "linear-gradient(135deg,#064E3B,#0A6B50)", color: "#fff" }}>
        <div>
      ...
      <div style={{ flex: 1, minHeight: 460, position: "relative" }}>
        {loading ? (
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", color: "#7A8F87", gap: 10 }}>
            <span style={{ fontSize: 22 }}>📍</span> Loading report locations…
          </div>
        ) : totalPlots === 0 ? (
          <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "#7A8F87", gap: 10 }}>
        ...
        {!loading && totalPlots > 0 && (
          <div style={{ position: "absolute", bottom: 16, right: 12, zIndex: 1000, background: "rgba(255,255,255,0.95)", borderRadius: 10, padding: "10px 14px", boxShadow: "0 2px 10px rgba(0,0,0,0.15)", fontSize: 12 }}>
            <div style={{ fontWeight: 700, marginBottom: 6, color: "#064E3B" }}>Counter</div>
            {[["1–4", "#10B981"], ["5–9", "#e67e22"], ["10+", "#c0392b"]].map(([label, color]) => (
      ...
      {selected && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.5)", backdropFilter: "blur(2px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, padding: 16 }} onClick={() => setSelected(null)}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 480, boxShadow: "0 24px 64px rgba(0,0,0,0.3)", overflow: "hidden", maxHeight: "85vh", display: "flex", flexDirection: "column" }}>
            <div style={{ background: "linear-gradient(135deg,#064E3B,#0A6B50)", padding: "16px 22px", color: "#fff", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
              <div>
            ...
            <div style={{ overflowY: "auto", flex: 1 }}>
              {selected.items.map((item, i) => (
                <div key={i} style={{ padding: "14px 22px", borderBottom: "1px solid #EDF2EF" }}>
                  <div style={{ fontWeight: 700, fontSize: 13, color: "#064E3B", marginBottom: 6 }}>Plot {item.plotNo || `#${i+1}`}</div>
                  {[
                    ...
                    <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", fontSize: 12 }}>
                      <span style={{ color: "#7A8F87", fontWeight: 600 }}>{label}</span>
                      <span style={{ color: "#3D5A50", fontWeight: 600, textAlign: "right", maxWidth: 260, wordBreak: "break-word" }}>{value}</span>
                    </div>
                  ))}
                </div>
              ))}
...
// Light KPI card adopted from the Valura concept: colored icon tile,
// mono uppercase label, Sora number. `tone` picks the accent from TONE.
const CARD_SHADOW      = "0 1px 2px rgba(6, 78, 59,.05), 0 6px 20px -12px rgba(6, 78, 59,.18)";
const CARD_SHADOW_HOVER = "0 10px 26px -12px rgba(6, 78, 59,.28)";
function StatCard({ label, value, tone = "blue", icon, delta, onClick }) {
  const t = TONE[tone] || TONE.blue;
  const big = String(value ?? "").length > 10;
  return (
    <div onClick={onClick} style={{
      ...
      onMouseEnter={e => { if (onClick) { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = CARD_SHADOW_HOVER; } }}
      onMouseLeave={e => { if (onClick) { e.currentTarget.style.transform = ""; e.currentTarget.style.boxShadow = CARD_SHADOW; } }}
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ width: 32, height: 32, borderRadius: 9, display: "grid", placeItems: "center", background: t.wash, color: t.ic, fontSize: 16 }}>{icon}</div>
        {delta != null && (
          <span style={{ fontSize: 11.5, fontFamily: FONT_MONO, fontWeight: 600, color: delta >= 0 ? C.success : C.danger, background: delta >= 0 ? "#E8F5EC" : "#fdeceb", borderRadius: 999, padding: "2px 8px" }}>
            {delta >= 0 ? "▲" : "▼"} {Math.abs(delta)}%
          </span>
        )}
      </div>
      <p style={{ margin: "12px 0 0", fontSize: 11, fontFamily: FONT_MONO, fontWeight: 500, letterSpacing: "0.06em", textTransform: "uppercase", color: C.muted }}>{label}</p>
      <p style={{ margin: "5px 0 0", fontFamily: FONT_DISPLAY, fontSize: big ? 20 : 28, fontWeight: 700, lineHeight: 1, color: C.navy, fontVariantNumeric: "tabular-nums" }}>{value}</p>
    </div>
  );
}

// Concept status chip: mono, pill, soft wash. `tone` from TONE.
function Chip({ tone = "blue", children }) {
  const t = TONE[tone] || TONE.blue;
  return (
    <span style={{
      display: "inline-block", fontFamily: FONT_MONO, fontSize: 10.5, fontWeight: 600,
      letterSpacing: "0.02em", color: t.ink, background: t.wash,
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 3 }}>
        <span style={{ color: "#5C7A6E", fontWeight: 600 }}>{label}</span>
        <span style={{ color: "#7A8F87" }}>{fmtBytes(bytes)} ({pct.toFixed(1)}%)</span>
      </div>
      <div style={{ background: "#D6DCE5", borderRadius: 6, height: 8, overflow: "hidden" }}>
...
function StorageCard({ stats }) {
  ...
  const C2 = { navy: "#064E3B", border: "#D6DCE5", muted: "#7A8F87" };
  ...
  return (
    <div>
      ...
      <div style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C2.border}`, padding: "20px 24px", marginBottom: 16 }}>
        ...
        <StorageBar label="Reports (with photos)" bytes={bd.reports || 0}    total={total} color="#10B981" />
        <StorageBar label="Version History"        bytes={bd.versions || 0}   total={total} color="#8e44ad" />
        <StorageBar label="Letterhead Image"       bytes={bd.letterhead || 0} total={total} color="#10B981" />
        ...
        <div style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C2.border}`, overflow: "hidden", marginBottom: 16 }}>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              ...
              <tbody>
                {files.map((f, i) => (
                  <tr key={f.id} style={{ borderBottom: `1px solid ${C2.border}`, background: i % 2 === 0 ? "#fff" : "#F8FAF9" }}>
                    ...
                    <td style={{ padding: "8px 14px", color: "#10B981" }}>{fmtBytes(f.report_bytes)}</td>
                    <td style={{ padding: "8px 14px", color: "#8e44ad" }}>{fmtBytes(f.version_bytes)}</td>
                    <td style={{ padding: "8px 14px", fontWeight: 700, color: C2.navy }}>
                      {fmtBytes(f.total)}
                      <div style={{ background: "#D6DCE5", borderRadius: 3, height: 4, marginTop: 3, overflow: "hidden" }}>
                        <div style={{ width: total > 0 ? Math.min(100, (f.total / total) * 100) + "%" : "0%", height: "100%", background: "#10B981", borderRadius: 3 }} />
                      </div>
                    </td>
      ...
      {(bd.letterhead || 0) > 0 && (
        <div style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C2.border}`, padding: "12px 20px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <span style={{ fontWeight: 700, color: "#10B981" }}>{fmtBytes(bd.letterhead)}</span>
        </div>
      )}
...
export default function AdminDashboard({ user, onLogout, onOpen }) {
  ...
  // Sidebar nav groups: all expanded by default
  const [openGroups, setOpenGroups] = useState({ overview: true, management: true, data: true, company: true, map: true });
  ...
  const [billFrom, setBillFrom] = useState("");   // received-date filter: from (YYYY-MM-DD)
  const [billTo, setBillTo]     = useState("");   // received-date filter: to (YYYY-MM-DD)
  ...
  const togglePause = async (u) => {
    const pausing = u.is_active !== 0;   // currently active → we're pausing
    if (pausing && !window.confirm(`Pause "${u.username}"? They will be signed out and unable to log in until resumed.`)) return;
    setPausingId(u.id);
    try { await api.setUserActive(u.id, !pausing); await loadUsers(); }
    catch (err) { alert(err.message || "Could not update the account."); }
  ...
  const NAV_GROUPS = [
    { id: "overview", label: "Reports & Billing", icon: "📊", items: [
      ["reports",    "📊", "Report Analytics"],
      ["billing",    "🧾", "Billing"],
    ]},
    { id: "management", label: "Management", icon: "🗂️", items: [
    ...
    { id: "data", label: "Data & Storage", icon: "💾", items: [
      ["storage",    "💾", "Storage"],
      ["field",      "📱", "Field Data"],
    ...
    { id: "map", label: "Rate Map", icon: "🗺️", items: [
      ["ratemap",    "🗺️", "Rate Map"],
    ]},
  ];

  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(180deg,#F8FAF9 0%,#F3F6F5 260px,#F3F6F5 100%)", fontFamily: FONT_UI }}>

      {/* ── Credit Balance Modal ── */}
      {creditModal && credits && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.6)", backdropFilter: "blur(3px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9100 }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "28px 32px", width: "96%", maxWidth: 680, maxHeight: "88vh", overflowY: "auto", boxShadow: "0 24px 64px rgba(0,0,0,0.32)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
              <h3 style={{ margin: 0, color: C.navy, fontSize: 18, fontWeight: 700, fontFamily: FONT_DISPLAY, letterSpacing: "-0.01em" }}>🪙 Credit Balance</h3>
              <button onClick={() => setCreditModal(false)} style={{ width: 32, height: 32, border: "none", borderRadius: 8, background: "#F3F6F5", cursor: "pointer", fontSize: 16, color: C.muted }}>✕</button>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 24 }}>
              {[
                ...
                { label: "Expiry", value: credits.expiry ? fmtLocal(credits.expiry) : "No Expiry", bg: "linear-gradient(135deg,#064E3B,#0A6B50)" },
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                  <thead>
                    <tr>{["Date", "Action", "Amount", "Balance", "By", "Note"].map(h => (
                      <th key={h} style={{ padding: "9px 12px", textAlign: "left", fontSize: 10, fontWeight: 700, color: C.muted, textTransform: "uppercase", background: "#F8FAF9", borderBottom: `2px solid ${C.border}` }}>{h}</th>
                    ))}</tr>
                  </thead>
                  <tbody>
                    {(credits.history || []).map((tx, i) => {
                      ...
                      const noteText = tx.report_id
                        ...
                        : tx.note || "—";
                      return (
                        <tr key={tx.id} style={{ background: i % 2 === 0 ? "#fff" : "#F8FAF9" }}>
                          <td style={{ padding: "9px 12px", color: C.muted, whiteSpace: "nowrap" }}>{fmtLocal(tx.created_at)}</td>
                          <td style={{ padding: "9px 12px" }}>
                            <span style={{ background: isDeduct ? "#fff5f5" : "#E8F5EC", color: isDeduct ? "#c0392b" : "#0E9E6E", borderRadius: 5, padding: "2px 8px", fontSize: 11, fontWeight: 700 }}>
                              {isDeduct ? "▼ deduct" : "▲ credit"}
                            </span>
                          </td>
                          <td style={{ padding: "9px 12px", whiteSpace: "nowrap" }}>
                            <span style={{ fontWeight: 800, fontSize: 14, color: isDeduct ? "#e74c3c" : "#10B981" }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.6)", backdropFilter: "blur(3px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9100 }}>
          <div style={{ background: "#fff", borderRadius: 14, padding: "28px 32px", width: "96%", maxWidth: 860, maxHeight: "88vh", overflowY: "auto", boxShadow: "0 24px 64px rgba(0,0,0,0.32)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
              <div>
                <h3 style={{ margin: 0, color: C.navy, fontSize: 17, fontWeight: 700, fontFamily: FONT_DISPLAY, letterSpacing: "-0.01em" }}>Report Changes</h3>
                <p style={{ margin: "4px 0 0", color: C.muted, fontSize: 12 }}>{diffModal.label}</p>
              </div>
              <button onClick={() => setDiffModal(null)} style={{ width: 30, height: 30, border: "none", borderRadius: 8, background: "#F3F6F5", cursor: "pointer", fontSize: 15, color: C.muted }}>✕</button>
            </div>
            {diffModal.loading ? (
                ...
                <thead>
                  <tr>
                    {["Field", "Before (this version)", "After (next save)"].map(h => (
                      <th key={h} style={{ padding: "9px 14px", textAlign: "left", background: "#F3F6F5", borderBottom: `2px solid ${C.border}`, fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: "0.4px" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {diffModal.rows.map((row, i) => (
                    <tr key={i} style={{ background: i % 2 === 0 ? "#fff" : "#F8FAF9", borderBottom: `1px solid ${C.border}` }}>
                      ...
                      <td style={{ padding: "9px 14px", color: row.type === "added" ? C.muted : C.danger, background: row.type === "added" ? "#F8FAF9" : "#fff5f5", maxWidth: 280, wordBreak: "break-word" }}>
                      ...
                      <td style={{ padding: "9px 14px", color: row.type === "removed" ? C.muted : C.success, background: row.type === "removed" ? "#F8FAF9" : "#E8F5EC", maxWidth: 280, wordBreak: "break-word" }}>
      ...
      {deleteConfirm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.6)", backdropFilter: "blur(3px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9200 }}>
      ...
      <div style={{
        ...
        color: C.navy, padding: "0 32px",
        display: "flex", alignItems: "center", justifyContent: "space-between",
        height: 60, position: "sticky", top: 0, zIndex: 100,
        borderBottom: "1px solid #D6DCE5",
        boxShadow: "0 2px 14px rgba(6, 78, 59,0.06)",
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <Wordmark size={32} />
          <span style={{ color: "#D6DCE5", fontSize: 18 }}>/</span>
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, lineHeight: 1.2, color: C.navy, fontFamily: FONT_DISPLAY, letterSpacing: "-0.01em" }}>{profile?.company_name || user.companyCode}</div>
            <div style={{ fontSize: 9, color: "rgba(6, 78, 59,0.4)", letterSpacing: "0.3px" }}><DevCredit style={{ color: "rgba(6, 78, 59,0.4)" }} /></div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ background: "linear-gradient(135deg,#f39c12,#e67e22)", color: "#fff", borderRadius: 5, padding: "3px 10px", fontSize: 10, fontWeight: 800, letterSpacing: "0.6px", boxShadow: "0 2px 6px rgba(230,126,34,0.28)" }}>ADMIN</span>
          <div style={{ display: "flex", alignItems: "center", gap: 7, background: "#F3F6F5", border: "1px solid #D6DCE5", borderRadius: 20, padding: "4px 12px 4px 5px" }}>
            <div style={{ width: 26, height: 26, borderRadius: "50%", background: "linear-gradient(135deg,#10B981,#0E9E6E)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, color: "#fff" }}>
              {user.username.charAt(0).toUpperCase()}
            </div>
            <span style={{ fontSize: 13, color: C.text, fontWeight: 600 }}>{user.username}</span>
          </div>
          <button onClick={onLogout}
            style={{ background: "#fff", color: C.danger, border: "1.5px solid #f3c6c1", borderRadius: 7, padding: "6px 14px", cursor: "pointer", fontSize: 12, fontWeight: 700, transition: "all 0.15s" }}
            ...
            onMouseLeave={e => { e.currentTarget.style.background = "#fff"; e.currentTarget.style.color = C.danger; e.currentTarget.style.borderColor = "#f3c6c1"; }}
          >Sign Out</button>
        </div>
      </div>

      {/* ── Hero banner ── */}
      <div style={{ background: "transparent", color: C.navy, padding: "28px 32px 0" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 16, marginBottom: 24 }}>
            <div>
              <p style={{ margin: 0, fontSize: 11.5, color: C.blue, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.18em", fontFamily: FONT_MONO }}>Admin Dashboard</p>
              <h2 style={{ margin: "6px 0 0", fontSize: 27, fontWeight: 700, letterSpacing: "-0.02em", color: C.navy, fontFamily: FONT_DISPLAY }}>
                {profile?.company_name || user.companyCode}
              </h2>
            </div>
            {credits !== null && (() => {
              const low = credits.balance <= (credits.low_threshold ?? 5);
              return (
              <div
                onClick={() => setCreditModal(true)}
                style={{ display: "flex", alignItems: "center", gap: 14, background: "#fff", border: `1px solid ${low ? "#f3c6c1" : "#f0e0b8"}`, borderRadius: 14, padding: "12px 20px", cursor: "pointer", boxShadow: "0 6px 18px rgba(6, 78, 59,0.08)", transition: "box-shadow 0.15s, transform 0.15s" }}
                onMouseEnter={e => { e.currentTarget.style.boxShadow = "0 10px 26px rgba(6, 78, 59,0.14)"; e.currentTarget.style.transform = "translateY(-2px)"; }}
                onMouseLeave={e => { e.currentTarget.style.boxShadow = "0 6px 18px rgba(6, 78, 59,0.08)"; e.currentTarget.style.transform = "none"; }}
              >
                <div style={{ width: 44, height: 44, borderRadius: 12, background: low ? "linear-gradient(135deg,#e74c3c,#c0392b)" : "linear-gradient(135deg,#f39c12,#e67e22)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, boxShadow: low ? "0 4px 14px rgba(231,76,60,0.32)" : "0 4px 14px rgba(230,126,34,0.32)" }}>🪙</div>
                <div>
                  <div style={{ fontSize: 10, fontWeight: 700, color: "#7A8F87", textTransform: "uppercase", letterSpacing: "0.6px" }}>Available Credits</div>
                  <div style={{ fontSize: 30, fontWeight: 900, lineHeight: 1.1, color: low ? "#e74c3c" : "#e67e22" }}>{credits.balance}</div>
                  {low && <div style={{ fontSize: 10, color: "#e74c3c", fontWeight: 700, marginTop: 1 }}>⚠ Low balance</div>}
                </div>
                <div style={{ fontSize: 15, color: "#C8D6CD", marginLeft: 4 }}>→</div>
              </div>
              );
            })()}
          </div>
      ...
      <div style={{ padding: "0 32px 32px", maxWidth: 1200, margin: "0 auto", display: "flex", gap: 24, alignItems: "flex-start" }}>
        ...
        <div style={{
          ...
          boxShadow: "0 6px 20px rgba(6, 78, 59,0.07)",
          position: "sticky", top: 76, alignSelf: "flex-start",
          overflow: "hidden", padding: "8px 0",
        }}>
          {NAV_GROUPS.map((group, gi) => {
            const isOpen = openGroups[group.id];
            const groupHasActive = group.items.some(([t]) => t === tab);
            return (
              <div key={group.id} style={{ padding: gi === 0 ? "0 0 2px" : "2px 0", borderTop: gi === 0 ? "none" : "1px solid #EDF2EF" }}>
                {/* Group header */}
                <button onClick={() => toggleGroup(group.id)} style={{
                  width: "calc(100% - 12px)", margin: "4px 6px 2px", padding: "8px 12px",
                ...
                >
                  ...
                  <span style={{
                    ...
                    textTransform: "uppercase", color: groupHasActive ? C.blue : "#7A8F87",
                  }}>{group.label}</span>
                  <span style={{
                    fontSize: 9, color: "#A8BAB0",
                ...
                {/* Sub-items */}
                {isOpen && group.items.map(([t, icon, lbl]) => {
                  const isActive = tab === t;
                  return (
                    <button key={t} onClick={() => setTab(t)} style={{
                      width: "calc(100% - 12px)", margin: "1px 6px", padding: "9px 12px 9px 26px",
                      border: "none", cursor: "pointer", borderRadius: 9,
                      fontSize: 13, fontWeight: isActive ? 700 : 500,
                      background: isActive ? "linear-gradient(135deg,#E8F5EC,#E8F5EC)" : "transparent",
                      ...
                      onMouseEnter={e => !isActive && (e.currentTarget.style.background = "#F8FAF9")}
        ...
        ══════════════════════════════════════════ */}
        {tab === "users" && (
          <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 1px 2px rgba(6, 78, 59,.05), 0 6px 20px -12px rgba(6, 78, 59,.18)" }}>
            {/* Card header */}
            <div style={{
              padding: "18px 24px",
              ...
              <div>
                <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: C.navy, fontFamily: FONT_DISPLAY, letterSpacing: "-0.01em" }}>Users</h2>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tr>
                  {["Username", "Email", "Status", "Created", "Actions"].map(h => (
                    <th key={h} style={{
                      ...
                      background: "#F8FAF9", borderBottom: `2px solid ${C.border}`,
                    }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {users.length === 0 ? (
                  ...
                  <tr key={u.id} style={{ background: i % 2 === 0 ? "#fff" : "#F8FAF9", transition: "background 0.12s" }}
                  ...
                  >
                    ...
                    <td style={{ padding: "13px 18px" }}>
                      {u.is_active === 0
                        ...
                        ? <Chip tone="blue">TEMP PASSWORD</Chip>
                    ...
                    <td style={{ padding: "13px 18px" }}>
                      <div style={{ display: "flex", gap: 8 }}>
                        <button
                          ...
                          title={u.is_active === 0 ? "Allow this user to sign in again" : "Prevent this user from signing in"}
                          style={{
                            background: u.is_active === 0 ? "#E8F5EC" : "#fbf1dc",
                            color: u.is_active === 0 ? C.success : "#a5691a",
                            border: "none", borderRadius: 6, padding: "6px 14px",
                            cursor: pausingId === u.id ? "wait" : "pointer", fontSize: 12, fontWeight: 600,
                            opacity: pausingId === u.id ? 0.6 : 1,
                          }}
                        >{pausingId === u.id ? "…" : u.is_active === 0 ? "▶ Resume" : "⏸ Pause"}</button>
                        <button
                          onClick={() => openModal("editUser", u)}
                          style={{ background: "#E8F5EC", color: C.blue, border: "none", borderRadius: 6, padding: "6px 14px", cursor: "pointer", fontSize: 12, fontWeight: 600 }}
                        >Edit</button>
                        <button
        ...
        {tab === "valuators" && (
          <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 1px 2px rgba(6, 78, 59,.05), 0 6px 20px -12px rgba(6, 78, 59,.18)" }}>
            <div style={{ padding: "20px 28px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
              <div>
                <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: C.navy, fontFamily: FONT_DISPLAY, letterSpacing: "-0.01em" }}>👷 Company Valuators</h2>
            ...
            {/* Inline edit form */}
            {editingValuator && (
              <div style={{ padding: "20px 28px", background: "#E8F5EC", borderBottom: `1px solid ${C.border}` }}>
                ...
                <thead>
                  <tr>
                    {["Name", "License No.", "Company / Firm", "Phone", "Email", "Actions"].map(h => (
                      <th key={h} style={{ padding: "10px 20px", textAlign: "left", fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase", borderBottom: `1px solid ${C.border}`, background: "#F8FAF9" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {valuatorList.map((v, i) => (
                    <tr key={v.id} style={{ background: i % 2 === 0 ? "#fff" : "#F8FAF9" }}>
            ...
            <div style={{ padding: "14px 28px", background: "#F8FAF9", borderTop: `1px solid ${C.border}`, fontSize: 12, color: C.muted, borderRadius: "0 0 14px 14px" }}>
        ...
        {tab === "banks" && (
          <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 1px 2px rgba(6, 78, 59,.05), 0 6px 20px -12px rgba(6, 78, 59,.18)" }}>
            <div style={{ padding: "20px 28px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
              <div>
                <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: C.navy, fontFamily: FONT_DISPLAY, letterSpacing: "-0.01em" }}>🏦 Company Bank List</h2>
                <p style={{ margin: "4px 0 0", fontSize: 13, color: C.muted }}>
            ...
            {bankList.length === 0 ? (
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {bankList.map((b, i) => (
                    <tr key={b} style={{ background: i % 2 === 0 ? "#fff" : "#F8FAF9" }}>
          ...
          <div>
            <div style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C.border}`, overflow: "hidden", marginBottom: 24 }}>
              <div style={{ padding: "20px 28px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: C.navy, fontFamily: FONT_DISPLAY, letterSpacing: "-0.01em" }}>💳 Payment Methods</h3>
                  <p style={{ margin: "4px 0 0", fontSize: 13, color: C.muted }}>
              ...
              {paymentMethodMsg && (
                <div style={{ padding: "10px 28px", background: paymentMethodMsg.startsWith("Error") ? "#fff5f5" : "#E8F5EC", color: paymentMethodMsg.startsWith("Error") ? C.danger : C.success, fontSize: 13, fontWeight: 600, borderBottom: `1px solid ${C.border}` }}>
                  {paymentMethodMsg}
                </div>
              )}
                ...
                <div style={{ padding: "16px 28px", display: "flex", flexDirection: "column", gap: 12 }}>
                  {paymentMethodList.map((pm, i) => (
                    <div key={pm.id} style={{ border: `1px solid ${C.border}`, borderRadius: 10, overflow: "hidden", display: "flex", alignItems: "stretch" }}>
                      ...
                      {pm.qrCode && (
                        <div style={{ padding: "10px 14px", background: "#F8FAF9", borderLeft: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <div>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", flex: 1 }}>
                  {allBankNames.map(bk => (
                    <button key={bk} onClick={() => setFeeTierBank(bk)}
                      style={{ padding: "6px 14px", borderRadius: 20, border: `2px solid ${feeTierBank === bk ? "#064E3B" : C.border}`, background: feeTierBank === bk ? "#064E3B" : "#fff", color: feeTierBank === bk ? "#fff" : C.text, fontWeight: 600, fontSize: 12, cursor: "pointer", whiteSpace: "nowrap" }}>
                      {bk}
                      {feeTiersMap[bk] && feeTiersMap[bk].length > 0 && (
                        <span style={{ marginLeft: 5, background: feeTierBank === bk ? "rgba(255,255,255,0.3)" : "#E8F5EC", color: feeTierBank === bk ? "#fff" : "#10B981", borderRadius: 10, padding: "1px 6px", fontSize: 10, fontWeight: 700 }}>{feeTiersMap[bk].length}</span>
                      )}
                    </button>
                  ))}
                </div>
              ...
              <div style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C.border}`, overflow: "hidden", marginBottom: 24 }}>
                <div style={{ padding: "14px 24px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
                  <div>
                    <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: C.navy, fontFamily: FONT_DISPLAY, letterSpacing: "-0.01em" }}>
                      {`🏦 ${feeTierBank}`}
                    </h3>
                      ...
                      <thead>
                        <tr>
                          {["#", "Label", "Up To FMV (NPR)", "Base Fee (NPR)", "Rate (0.000 format)", "Actions"].map(h => (
                            <th key={h} style={{ padding: "10px 16px", textAlign: "left", fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase", borderBottom: `1px solid ${C.border}`, background: "#F8FAF9", whiteSpace: "nowrap" }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {activeTiers.map((t, i) => (
                          <tr key={i} style={{ borderBottom: `1px solid ${C.border}`, background: i % 2 === 0 ? "#fff" : "#F8FAF9" }}>
              ...
              {/* Edit / Add tier modal */}
              {editingTier && (
                <div style={{ position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.65)", backdropFilter: "blur(3px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9500 }}>
        ...
        {tab === "billing" && (() => {
          ...
          const dateActive = !!(billFrom || billTo);
          // Filter by RECEIVED date (payment date). Rows without a received date
          // are excluded whenever a date bound is set.
          const rows = dateActive
            ? allRows.filter(r => {
                if (!r.receivedAt) return false;
                if (billFrom && r.receivedAt < billFrom) return false;
                if (billTo   && r.receivedAt > billTo)   return false;
                return true;
              })
            : allRows;
          const sumBilled      = rows.reduce((s, r) => s + (r.grandTotal || 0), 0);
          const sumReceived    = rows.reduce((s, r) => s + (r.received || 0), 0);
          const sumOutstanding = Math.max(0, sumBilled - sumReceived);
          const fmtRange = () => {
            if (billFrom && billTo) return `${billFrom} → ${billTo}`;
            if (billFrom) return `from ${billFrom}`;
            if (billTo)   return `up to ${billTo}`;
            return "";
          };
          const inputStyle = { padding: "8px 11px", border: `1.5px solid ${C.border}`, borderRadius: 8, fontSize: 13, color: C.text, background: "#fff", fontFamily: "inherit", outline: "none" };

          const doPrintBilling = () => {
            const esc = (v) => String(v ?? "").replace(/[&<>]/g, c => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;" }[c]));
            const nf = (n) => (n || 0).toLocaleString("en-NP");
            const company = profile?.company_name || user.companyCode;
            const periodLabel = dateActive
              ? `Payments received ${fmtRange()}`
              : "All billing records";
            const genAt = new Date().toLocaleString(undefined, { year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hour12: false });
            const bodyRows = rows.map((r, i) => `
              <tr>
                <td>${i + 1}</td>
                <td class="l">${esc(r.clientName)}</td>
                <td class="l">${esc(r.bank)}</td>
                <td>${esc(r.reportDate)}</td>
                <td class="r">${r.fmv > 0 ? nf(r.fmv) : "—"}</td>
                <td class="r">${r.grandTotal > 0 ? nf(r.grandTotal) : "—"}</td>
                <td class="r rec">${r.received > 0 ? nf(r.received) : "—"}</td>
                <td>${esc(r.receivedAt || "—")}</td>
                <td class="l">${esc(r.receivedBank || "—")}</td>
                <td class="l">${esc(r.receivedBy || "—")}</td>
                <td class="r">${r.outstanding > 0 ? nf(r.outstanding) : "Paid"}</td>
              </tr>`).join("");
            const html = `<!doctype html><html><head><meta charset="utf-8"><title>Billing Record — ${esc(company)}</title>
              <style>
                *{box-sizing:border-box} body{font-family:'Segoe UI',Arial,sans-serif;color:#064E3B;margin:28px;font-size:12px}
                h1{font-size:19px;margin:0 0 2px;color:#064E3B} .sub{color:#555;font-size:12px;margin:0 0 2px}
                .meta{color:#777;font-size:11px;margin:0 0 16px}
                .cards{display:flex;gap:10px;margin:0 0 16px;flex-wrap:wrap}
                .card{border:1px solid #D6DCE5;border-radius:8px;padding:8px 14px;min-width:150px}
                .card .k{font-size:9.5px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:#777}
                .card .v{font-size:17px;font-weight:800;color:#064E3B;margin-top:3px}
                .card .v.green{color:#0E9E6E} .card .v.orange{color:#d97706}
                table{width:100%;border-collapse:collapse;font-size:11px}
                th,td{border:1px solid #C8D6CD;padding:6px 8px;text-align:center;white-space:nowrap}
                th{background:#EDF2EF;font-size:9.5px;text-transform:uppercase;letter-spacing:.4px;color:#333}
                td.l{text-align:left} td.r{text-align:right} td.rec{color:#0E9E6E;font-weight:700}
                tfoot td{font-weight:800;background:#EDF2EF;color:#064E3B}
                .foot{margin-top:20px;font-size:10px;color:#888;text-align:center}
                @media print{@page{size:A4 landscape;margin:12mm} body{margin:0}}
              </style></head><body>
              <h1>${esc(company)} — Billing Record</h1>
              <p class="sub">${esc(periodLabel)}</p>
              <p class="meta">Generated ${esc(genAt)}</p>
              <div class="cards">
                <div class="card"><div class="k">${dateActive ? "Reports in Period" : "Total Reports"}</div><div class="v">${rows.length}</div></div>
                <div class="card"><div class="k">Total Billed</div><div class="v">NPR ${nf(sumBilled)}</div></div>
                <div class="card"><div class="k">${dateActive ? "Received in Period" : "Total Received"}</div><div class="v green">NPR ${nf(sumReceived)}</div></div>
                <div class="card"><div class="k">Outstanding</div><div class="v ${sumOutstanding > 0 ? "orange" : "green"}">NPR ${nf(sumOutstanding)}</div></div>
              </div>
              <table>
                <thead><tr>
                  <th>#</th><th>Client</th><th>Bank</th><th>Report Date</th><th>FMV (NPR)</th>
                  <th>Bill Amount</th><th>Received</th><th>Received At</th><th>Received In Bank</th><th>Received By</th><th>Outstanding</th>
                </tr></thead>
                <tbody>${bodyRows || `<tr><td colspan="11">No records</td></tr>`}</tbody>
                <tfoot><tr>
                  <td colspan="5">TOTAL${dateActive ? " (PERIOD)" : ""}</td>
                  <td class="r">${nf(sumBilled)}</td>
                  <td class="r">${nf(sumReceived)}</td>
                  <td colspan="3"></td>
                  <td class="r">${sumOutstanding > 0 ? nf(sumOutstanding) : "All Paid"}</td>
                </tr></tfoot>
              </table>
              <p class="foot">${esc(company)} · Printed from Mulyanka</p>
              </body></html>`;
            const w = window.open("", "_blank");
            if (!w) { alert("Please allow pop-ups to print the billing record."); return; }
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 14, marginBottom: 20 }}>
              {[
                { label: dateActive ? "Reports in Period" : "Total Reports", value: billingStats ? rows.length : "—", tone: "navy", icon: "📋" },
                { label: "Total Billed",  value: billingStats ? `NPR ${sumBilled.toLocaleString("en-NP")}` : "—", tone: "blue",  icon: "🧾" },
                { label: dateActive ? "Received in Period" : "Total Received", value: billingStats ? `NPR ${sumReceived.toLocaleString("en-NP")}` : "—", tone: "green", icon: "✅" },
                { label: "Outstanding",  value: billingStats ? `NPR ${sumOutstanding.toLocaleString("en-NP")}` : "—", tone: sumOutstanding > 0 ? "amber" : "green", icon: "⏳" },
              ].map(({ label, value, tone, icon }) => (
                <StatCard key={label} label={label} value={billingLoading ? "…" : value} tone={tone} icon={icon} />
              ))}
            </div>

            {/* Date range filter bar */}
            <div style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C.border}`, padding: "14px 20px", marginBottom: 16, display: "flex", alignItems: "flex-end", gap: 16, flexWrap: "wrap" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, fontWeight: 700, color: C.navy }}>
                <span style={{ fontSize: 16 }}>📅</span> Filter by Received Date
              </div>
              <div>
                <label style={{ display: "block", fontSize: 10.5, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 5 }}>From</label>
                <input type="date" value={billFrom} max={billTo || undefined} onChange={e => setBillFrom(e.target.value)} style={inputStyle} />
              </div>
              <div>
                <label style={{ display: "block", fontSize: 10.5, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 5 }}>To</label>
                <input type="date" value={billTo} min={billFrom || undefined} onChange={e => setBillTo(e.target.value)} style={inputStyle} />
              </div>
              {dateActive && (
                <button onClick={() => { setBillFrom(""); setBillTo(""); }}
                  style={{ padding: "8px 16px", border: `1.5px solid ${C.border}`, borderRadius: 8, background: "#fff", cursor: "pointer", fontSize: 12.5, fontWeight: 600, color: C.text }}>
                  ✕ Clear
                </button>
              )}
              {dateActive && (
                <div style={{ marginLeft: "auto", fontSize: 12.5, color: C.muted }}>
                  Showing <strong style={{ color: C.navy }}>{rows.length}</strong> payment{rows.length !== 1 ? "s" : ""} received <strong style={{ color: C.navy }}>{fmtRange()}</strong>
                  {" · "}Total received <strong style={{ color: C.success }}>NPR {sumReceived.toLocaleString("en-NP")}</strong>
                </div>
              )}
            </div>
            ...
            <div style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C.border}`, overflow: "hidden" }}>
              <div style={{ padding: "16px 24px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: C.navy, fontFamily: FONT_DISPLAY, letterSpacing: "-0.01em" }}>🧾 Billing Details {dateActive ? "— Received in Period" : "per Report"}</h3>
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={doPrintBilling} disabled={billingLoading || rows.length === 0}
                    style={{ padding: "7px 16px", border: "none", borderRadius: 8, background: GRAD.blue, color: "#fff", cursor: rows.length === 0 ? "not-allowed" : "pointer", fontSize: 12, fontWeight: 700, opacity: rows.length === 0 ? 0.5 : 1, boxShadow: "0 2px 8px rgba(16, 185, 129,0.28)" }}>
                    🖨 Print
                  </button>
              ...
              {!billingStats || rows.length === 0 ? (
                <div style={{ padding: "48px 24px", textAlign: "center", color: C.muted, fontSize: 14, fontStyle: "italic" }}>
                  {billingLoading ? "Loading billing data…"
                    : dateActive ? "No payments received in the selected date range."
                    : "No billing data found. Save reports with billing fields to see stats here."}
                </div>
              ) : (
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                    ...
                    <tbody>
                      {rows.map((r, i) => (
                        <tr key={r.id} style={{ borderBottom: `1px solid ${C.border}`, background: i % 2 === 0 ? "#fff" : "#F8FAF9" }}>
                          ...
                          <td style={{ padding: "10px 16px", fontWeight: 700, color: r.outstanding > 0 ? C.warn : C.success }}>
                            {r.outstanding > 0 ? r.outstanding.toLocaleString("en-NP") : <Chip tone="green">✓ Paid</Chip>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot>
                      <tr style={{ background: "#E8F5EC", borderTop: `2px solid ${C.navy}` }}>
                        <td colSpan={5} style={{ padding: "10px 16px", fontWeight: 700, color: C.navy }}>TOTAL{dateActive ? " (PERIOD)" : ""}</td>
                        <td style={{ padding: "10px 16px", fontWeight: 800, color: C.navy }}>{sumBilled.toLocaleString("en-NP")}</td>
                        <td style={{ padding: "10px 16px", fontWeight: 800, color: C.success }}>{sumReceived.toLocaleString("en-NP")}</td>
                        <td style={{ padding: "10px 16px" }}></td>
                        <td style={{ padding: "10px 16px" }}></td>
                        <td style={{ padding: "10px 16px" }}></td>
                        <td style={{ padding: "10px 16px", fontWeight: 800, color: sumOutstanding > 0 ? "#e67e22" : C.success }}>
        ...
        {tab === "storage" && (
          <div>
            {storageLoading
              ? <div style={{ textAlign: "center", padding: 40, color: "#7A8F87" }}>Loading storage data…</div>
              : storageError
              ? <div style={{ textAlign: "center", padding: 40 }}>
                  <div style={{ color: "#e74c3c", fontWeight: 700, marginBottom: 8 }}>❌ Failed to load storage data</div>
                  <div style={{ color: "#7A8F87", fontSize: 13, marginBottom: 16 }}>{storageError}</div>
                  <button onClick={() => { setStorageLoading(true); setStorageError(null); api.getStorageStats().then(d => { setStorageStats(d); setStorageLoading(false); }).catch(e => { setStorageError(e.message); setStorageLoading(false); }); }}
                    style={{ padding: "8px 20px", background: "#064E3B", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 700 }}>
                    ↺ Retry
                  </button>
                </div>
              : storageStats
          ...
          <div>
            ...
            {stats && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginBottom: 24 }}>
                <StatCard label="Total Reports"       value={stats.totals?.total ?? 0}       tone="navy"   icon="📋" />
                <StatCard label="Preliminary Reports" value={stats.totals?.preliminary ?? 0} tone="blue"   icon="📝"
                  onClick={() => setReportSubTab("preliminary")} />
                <StatCard label="Final Reports"       value={stats.totals?.final ?? 0}       tone="green"  icon="✅"
                  onClick={() => setReportSubTab("final")} />
              </div>
            )}

            {/* Sub-tab pills */}
            <div style={{ display: "flex", gap: 6, marginBottom: 18, background: "#fff", borderRadius: 10, padding: 5, boxShadow: "0 1px 2px rgba(6, 78, 59,.05), 0 4px 14px -10px rgba(6, 78, 59,.16)", width: "fit-content" }}>
              {[
                ["all",         "📋", "All Reports"],
                ...
                <button key={key} onClick={() => setReportSubTab(key)} style={{
                  ...
                  boxShadow: reportSubTab === key ? "0 2px 8px rgba(16, 185, 129,0.3)" : "none",
              ...
              return (
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 12, marginBottom: 16 }}>
                    {[
                      { label: "Total",       value: grandTotal.total,       color: "#064E3B", bg: "#E8F5EC",  icon: "📋" },
                      { label: "Preliminary", value: grandTotal.preliminary, color: "#10B981", bg: "#E8F5EC",  icon: "📝" },
                      { label: "Bill",        value: grandTotal.bill,        color: "#8e44ad", bg: "#f5eef8",  icon: "🧾" },
                      { label: "Final",       value: grandTotal.final,       color: "#10B981", bg: "#E8F5EC",  icon: "✅" },
                  ...
                  {/* Header + search */}
                  <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 1px 2px rgba(6, 78, 59,.05), 0 6px 20px -12px rgba(6, 78, 59,.18)", overflow: "hidden", marginBottom: selectedBank ? 20 : 0 }}>
                    <div style={{ padding: "16px 24px", borderBottom: `2px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
                      <div>
                        <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: C.navy, fontFamily: FONT_DISPLAY, letterSpacing: "-0.01em" }}>📊 Bank / Branch Analysis</h2>
                      ...
                      <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                        ...
                        <button onClick={() => { loadStats(); loadReports(reportSearch); setBankSearch(""); setSelectedBank(null); }}
                          style={{ padding: "8px 14px", background: "#F3F6F5", color: C.text, border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>↺ Refresh</button>
                      </div>
                    </div>
                    ...
                    {allBanks.length === 0 ? (
                      ...
                      return (
                        <div key={bankName} style={{ borderBottom: `1px solid ${C.border}` }}>
                          {/* Bank row */}
                          <div
                            style={{ padding: "13px 20px", background: isExpanded ? "#E8F5EC" : "#F8FAF9", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}
                          >
                            {/* Expand toggle */}
                            <button
                            ...
                            <div style={{ flex: 2, minWidth: 160 }}>
                              <div style={{ display: "flex", height: 16, borderRadius: 8, overflow: "hidden", background: "#D6DCE5" }}>
                                <div style={{ width: `${(bankData.preliminary / maxTotal) * 100}%`, background: "#10B981" }} title={`Preliminary: ${bankData.preliminary}`} />
                                <div style={{ width: `${((bankData.bill || 0) / maxTotal) * 100}%`, background: "#cf8f22" }} title={`Bill: ${bankData.bill || 0}`} />
                            ...
                            <div style={{ display: "flex", gap: 6, flexShrink: 0, flexWrap: "wrap", alignItems: "center" }}>
                              <span style={{ background: "#E8F5EC", color: "#10B981", borderRadius: 5, padding: "2px 8px", fontSize: 11, fontWeight: 700 }}>P: {bankData.preliminary}</span>
                              {bankData.bill > 0 && <span style={{ background: "#fff8ee", color: "#cf8f22", borderRadius: 5, padding: "2px 8px", fontSize: 11, fontWeight: 700 }}>B: {bankData.bill}</span>}
                              <span style={{ background: "#E8F5EC", color: C.success, borderRadius: 5, padding: "2px 8px", fontSize: 11, fontWeight: 700 }}>F: {bankData.final}</span>
                              ...
                              <span style={{ background: "#F3F6F5", color: C.text, borderRadius: 5, padding: "2px 8px", fontSize: 11, fontWeight: 700 }}>Total: {bankData.total}</span>
                              <button
                          ...
                          {isExpanded && branches.map(([branchName, branchData]) => (
                            <div key={branchName} style={{ padding: "8px 20px 8px 56px", display: "flex", alignItems: "center", gap: 12, borderTop: `1px solid ${C.border}`, flexWrap: "wrap", background: "#F8FAF9" }}>
                              ...
                              <div style={{ flex: 2, minWidth: 140 }}>
                                <div style={{ display: "flex", height: 10, borderRadius: 5, overflow: "hidden", background: "#D6DCE5" }}>
                                  <div style={{ width: `${(branchData.preliminary / maxTotal) * 100}%`, background: "#10B981" }} />
                                  <div style={{ width: `${((branchData.bill || 0) / maxTotal) * 100}%`, background: "#cf8f22" }} />
                              ...
                              <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
                                <span style={{ background: "#E8F5EC", color: "#10B981", borderRadius: 4, padding: "1px 7px", fontSize: 10, fontWeight: 700 }}>P: {branchData.preliminary}</span>
                                {branchData.bill > 0 && <span style={{ background: "#fff8ee", color: "#cf8f22", borderRadius: 4, padding: "1px 7px", fontSize: 10, fontWeight: 700 }}>B: {branchData.bill}</span>}
                                <span style={{ background: "#E8F5EC", color: C.success, borderRadius: 4, padding: "1px 7px", fontSize: 10, fontWeight: 700 }}>F: {branchData.final}</span>
                                ...
                                <span style={{ background: "#F3F6F5", color: C.text, borderRadius: 4, padding: "1px 7px", fontSize: 10, fontWeight: 700 }}>{branchData.total}</span>
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    <div style={{ padding: "10px 20px", background: "#F8FAF9", display: "flex", gap: 16, fontSize: 11, color: C.muted, flexWrap: "wrap", alignItems: "center" }}>
                      <span><span style={{ display: "inline-block", width: 10, height: 10, background: "#10B981", borderRadius: 2, marginRight: 4, verticalAlign: "middle" }}/>Preliminary</span>
                      <span><span style={{ display: "inline-block", width: 10, height: 10, background: "#cf8f22", borderRadius: 2, marginRight: 4, verticalAlign: "middle" }}/>Bill</span>
                  ...
                  {selectedBank && (
                    <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 1px 2px rgba(6, 78, 59,.05), 0 6px 20px -12px rgba(6, 78, 59,.18)", overflow: "hidden" }}>
                      <div style={{ padding: "14px 24px", borderBottom: `2px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <div>
                          <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: C.navy, fontFamily: FONT_DISPLAY, letterSpacing: "-0.01em" }}>🏦 {selectedBank} — Reports</h3>
                        ...
                        <button onClick={() => setSelectedBank(null)} style={{ background: "#F3F6F5", border: "none", borderRadius: 8, padding: "6px 14px", cursor: "pointer", fontSize: 12, fontWeight: 600, color: C.muted }}>✕ Close</button>
                      </div>
                      <div style={{ overflowX: "auto" }}>
                        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                          <thead>
                            <tr>
                              {["ID", "Client", "Branch", "Type", "Visit Date", "By", "Amt Received", "Received By", "Rejection Reason"].map(h => (
                                <th key={h} style={{ padding: "9px 12px", textAlign: "left", fontSize: 10, fontWeight: 700, color: C.muted, textTransform: "uppercase", borderBottom: `2px solid ${C.border}`, background: "#F8FAF9", whiteSpace: "nowrap" }}>{h}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody>
                            {bankReports.map((r, i) => (
                              <tr key={(r._isFieldSub ? "fs-" : "r-") + r.id} style={{ borderBottom: `1px solid ${C.border}`, background: r._isFieldSub ? (i % 2 === 0 ? "#fff8f8" : "#fff0f0") : (i % 2 === 0 ? "#fff" : "#F8FAF9") }}>
                                ...
                                <td style={{ padding: "8px 12px" }}>
                                  {r._isFieldSub ? (
                                    ...
                                    <span style={{
                                      background: r.report_type === "preliminary" ? "#E8F5EC" : r.report_type === "bill" ? "#fff8ee" : "#E8F5EC",
                                      color:      r.report_type === "preliminary" ? "#10B981" : r.report_type === "bill" ? "#cf8f22" : C.success,
                                ...
                                <td style={{ padding: "8px 12px", fontSize: 12, whiteSpace: "nowrap" }}>
                                  {r.report_type === "final" && r.amount_received != null && r.amount_received > 0
                                    ? <span style={{ fontWeight: 700, color: "#0E9E6E" }}>NPR {Number(r.amount_received).toLocaleString("en-NP")}</span>
            ...
            {viewRejectedSub && (() => {
              ...
              return (
                <div style={{ position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.55)", backdropFilter: "blur(2px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, padding: 16 }} onClick={() => setViewRejectedSub(null)}>
                  <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 440, boxShadow: "0 24px 64px rgba(0,0,0,0.3)", overflow: "hidden", fontFamily: "'Poppins', system-ui, -apple-system, 'Poppins', system-ui, -apple-system, 'Segoe UI', sans-serif", maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
                    {/* Header */}
              ...
              return (
                <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 1px 2px rgba(6, 78, 59,.05), 0 6px 20px -12px rgba(6, 78, 59,.18)" }}>
                  ...
                  <div style={{ padding: "18px 24px", borderBottom: `2px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
                    <div>
                      <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: C.navy, fontFamily: FONT_DISPLAY, letterSpacing: "-0.01em" }}>
                        {tabTitle}
                      </h2>
                        ...
                        <thead>
                          <tr>
                            {["#", "Client / Owner", "Bank", "Branch", "T", "Visit", "Report Dt", "By", "Saved", "Prints", "Edits", ""].map(h => (
                              <th key={h} style={{ padding: "7px 10px", textAlign: "left", fontSize: 10, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: "0.4px", background: "#F8FAF9", borderBottom: `2px solid ${C.border}`, whiteSpace: "nowrap" }}>{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {filtered.map((r, i) => {
                            ...
                            return (
                              <React.Fragment key={r.id}>
                                <tr style={{ background: i % 2 === 0 ? "#fff" : "#F8FAF9", cursor: r.prints.length > 0 ? "pointer" : "default", fontSize: 12 }}
                                  ...
                                  <td style={{ padding: "7px 10px" }}>
                                    <span title={r.report_type} style={{
                                      background: r.report_type === "preliminary" ? "#E8F5EC" : r.report_type === "bill" ? "#fff8ee" : r.report_type === "rejected" ? "#fdecea" : "#E8F5EC",
                                      color:      r.report_type === "preliminary" ? "#10B981" : r.report_type === "bill" ? "#cf8f22" : r.report_type === "rejected" ? "#c0392b" : C.success,
                                  ...
                                  <td style={{ padding: "7px 10px", whiteSpace: "nowrap" }}>
                                    <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                                      {prelPrints.length  > 0 && <span style={{ background: "#E8F5EC", color: "#10B981", borderRadius: 4, padding: "1px 6px", fontSize: 10, fontWeight: 700 }}>P×{prelPrints.length}</span>}
                                      {finalPrints.length > 0 && <span style={{ background: "#E8F5EC", color: C.success, borderRadius: 4, padding: "1px 6px", fontSize: 10, fontWeight: 700 }}>F×{finalPrints.length}</span>}
                                  ...
                                  <td style={{ padding: "7px 10px", whiteSpace: "nowrap" }} onClick={e => e.stopPropagation()}>
                                    <button
                                      ...
                                      style={{ background: "#E8F5EC", color: C.navy, border: `1px solid ${C.border}`, borderRadius: 5, padding: "3px 8px", cursor: "pointer", fontSize: 10, fontWeight: 700, whiteSpace: "nowrap" }}
                                    >
                                                ...
                                                return (
                                                  <tr key={v.id} style={{ background: vi % 2 === 0 ? "#fff" : "#f5f3ff" }}>
                                                    ...
                                                    <td style={{ padding: "6px 10px" }}>
                                                      {v.report_type && <span style={{ background: v.report_type === "preliminary" ? "#E8F5EC" : "#E8F5EC", color: v.report_type === "preliminary" ? "#10B981" : C.success, borderRadius: 4, padding: "1px 7px", fontSize: 10, fontWeight: 700 }}>{v.report_type}</span>}
                                                    </td>
                                ...
                                {isExpanded && (
                                  <tr>
                                    <td colSpan={13} style={{ padding: "0 10px 10px 32px", background: "#E8F5EC" }}>
                                      ...
                                      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                                        ...
                                        <tbody>
                                          {r.prints.map((p, pi) => (
                                            <tr key={pi} style={{ background: pi % 2 === 0 ? "#fff" : "#F8FAF9" }}>
                                              <td style={{ padding: "6px 10px", fontWeight: 600, color: C.navy }}>
                                              ...
                                              <td style={{ padding: "6px 10px" }}>
                                                <span style={{ background: p.print_type === "preliminary" ? "#E8F5EC" : "#E8F5EC", color: p.print_type === "preliminary" ? "#10B981" : C.success, borderRadius: 4, padding: "1px 7px", fontSize: 10, fontWeight: 700 }}>{p.print_type}</span>
                                              </td>
                                              <td style={{ padding: "6px 10px" }}>
            ...
            {reportSubTab === "billing" && (() => {
              const BILL_TYPES = [
                { key: "all",         label: "All",         icon: "📋", color: C.navy,    bg: "#E8F5EC" },
                { key: "preliminary", label: "Preliminary", icon: "📝", color: "#10B981", bg: "#E8F5EC" },
                { key: "bill",        label: "Bill",        icon: "🧾", color: "#cf8f22", bg: "#fff8ee" },
                { key: "final",       label: "Final",       icon: "✅", color: C.success, bg: "#E8F5EC" },
              ];
              const allRows = billingStats?.perReport || [];
              ...
              const typeStyle = (t) => ({
                background: t === "preliminary" ? "#E8F5EC" : t === "bill" ? "#fff8ee" : t === "final" ? "#E8F5EC" : "#F3F6F5",
                color:      t === "preliminary" ? "#10B981" : t === "bill" ? "#cf8f22" : t === "final" ? C.success  : C.muted,
              });
              ...
              return (
                <div>
                    ...
                    {BILL_TYPES.map(({ key, label, icon }) => {
                      ...
                      return (
                        <button key={key} onClick={() => setBillTypeFilter(key)} style={{
                          ...
                          boxShadow: isActive ? "0 2px 8px rgba(16, 185, 129,0.3)" : "0 1px 4px rgba(0,0,0,0.08)",
                          transition: "all 0.15s",
                          display: "flex", alignItems: "center", gap: 6,
                        }}>
                          {icon} {label}
                          <span style={{ background: isActive ? "rgba(255,255,255,0.25)" : "#F3F6F5", color: isActive ? "#fff" : C.muted, borderRadius: 10, padding: "1px 7px", fontSize: 11, fontWeight: 700 }}>{count}</span>
                        </button>
                      );
                    })}
                  </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(190px,1fr))", gap: 14, marginBottom: 24 }}>
                    {[
                      { label: "Reports",     value: filtered.length,                                                              tone: "navy",  icon: "📋" },
                      { label: "Total Billed",  value: billingStats ? `NPR ${sumBilled.toLocaleString("en-NP")}` : "—",           tone: "blue",  icon: "🧾" },
                      { label: "Total Received", value: billingStats ? `NPR ${sumReceived.toLocaleString("en-NP")}` : "—",        tone: "green", icon: "✅" },
                      { label: "Outstanding",  value: billingStats ? `NPR ${sumOutstanding.toLocaleString("en-NP")}` : "—",       tone: sumOutstanding > 0 ? "amber" : "green", icon: "⏳" },
                    ].map(({ label, value, tone, icon }) => (
                      <StatCard key={label} label={label} value={billingLoading ? "…" : value} tone={tone} icon={icon} />
                    ))}
                          ...
                          <thead>
                            <tr>
                              {["ID", "Client", "Bank", "Branch", "Type", "Bill No.", "Billed (NPR)", "Received (NPR)", "Outstanding (NPR)"].map(h => (
                                <th key={h} style={{ padding: "10px 14px", textAlign: "left", fontSize: 10, fontWeight: 700, color: C.muted, textTransform: "uppercase", borderBottom: `2px solid ${C.border}`, background: "#F8FAF9", whiteSpace: "nowrap" }}>{h}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody>
        ...
        {tab === "profile" && profile && (
          <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 1px 2px rgba(6, 78, 59,.05), 0 6px 20px -12px rgba(6, 78, 59,.18)" }}>
            <div style={{
              ...
              <div>
                <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: C.navy, fontFamily: FONT_DISPLAY, letterSpacing: "-0.01em" }}>Company Profile</h2>
            ...
            <form onSubmit={saveProfile} style={{ padding: "28px 28px 24px" }}>
              {profileMsg === "success" && (
                <div style={{ background: "#E8F5EC", color: C.success, border: "1px solid #A7E3B1", borderRadius: 8, padding: "11px 16px", fontSize: 14, marginBottom: 20, fontWeight: 600 }}>
              ...
              <div style={{ marginTop: 24, borderTop: `1.5px solid ${C.border}`, paddingTop: 20 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
                  ...
                  <FL label="Bank Account Details" span={2}>
                    <textarea value={profile.bank_account || ""} onChange={setP("bank_account")} rows={3} placeholder={"Bank Name: Nepal Bank Ltd.\nAccount No.: 1234567890\nBranch: Kathmandu"} style={{ width: "100%", padding: "10px 14px", border: `1.5px solid ${C.border}`, borderRadius: 8, fontSize: 14, boxSizing: "border-box", resize: "vertical", fontFamily: "'Poppins', system-ui, -apple-system, 'Segoe UI', sans-serif" }} />
                  </FL>
                </div>
              </div>
              ...
              <div style={{ marginTop: 8, display: "flex", justifyContent: "flex-end" }}>
                <button
                  ...
                  style={{
                    ...
                    boxShadow: profileSaving ? "none" : "0 2px 8px rgba(16, 185, 129,0.3)",
                  }}
      ...
      {(modal === "addUser" || modal === "editUser") && (
        <Modal title={modal === "addUser" ? "Add New User" : `Edit User — ${target?.username}`} onClose={closeModal}>
          ...
          <form onSubmit={saveUser}>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
              <button type="button" onClick={closeModal} style={{ background: "#F3F6F5", color: C.text, border: "none", borderRadius: 8, padding: "9px 18px", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>Cancel</button>
      ...
      {modal === "deleteUser" && (
        <Modal title="Delete User" onClose={closeModal} width={420}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            <button onClick={closeModal} style={{ background: "#F3F6F5", color: C.text, border: "none", borderRadius: 8, padding: "9px 18px", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>Cancel</button>
