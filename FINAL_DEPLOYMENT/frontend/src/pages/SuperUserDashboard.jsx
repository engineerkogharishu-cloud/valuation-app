...
// ── Design tokens ─────────────────────────────────────────────
const C = {
  navy: "#064E3B", blue: "#10B981", danger: "#e74c3c",
  success: "#10B981", warn: "#f39c12", purple: "#8e44ad",
  bg: "#F3F6F5", border: "#D6DCE5", text: "#3D5A50", muted: "#7A8F87",
};

const GRAD = {
  navy:    "linear-gradient(135deg, #064E3B 0%, #0A6B50 100%)",
  blue:    "linear-gradient(135deg, #10B981 0%, #0E9E6E 100%)",
...
const S = {
  page: { minHeight: "100vh", background: C.bg, fontFamily: "'Poppins', system-ui, -apple-system, 'Poppins', system-ui, -apple-system, 'Segoe UI', sans-serif" },
  ...
  header: {
    ...
    alignItems: "center",
    height: 72, boxShadow: "0 4px 24px rgba(6, 78, 59,0.45)",
  ...
  th: {
    ...
    borderBottom: `2px solid ${C.border}`, background: "#F8FAF9",
  },
  td: { padding: "12px 16px", fontSize: 13.5, color: C.text, borderBottom: `1px solid #F3F6F5` },
...
function Modal({ title, onClose, children, maxWidth = 520 }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9000, padding: 16, backdropFilter: "blur(2px)" }}>
      <div style={{ background: "#fff", borderRadius: 16, padding: "28px 32px", width: "100%", maxWidth, boxShadow: "0 32px 80px rgba(0,0,0,0.3)", maxHeight: "92vh", overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
          ...
          <button onClick={onClose} style={{ background: "#F3F6F5", border: "none", borderRadius: 7, width: 30, height: 30, cursor: "pointer", color: C.muted, fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
        </div>
        {children}
      </div>
    </div>
...
const Input = (props) => (
  <input {...props} style={{ width: "100%", padding: "10px 14px", border: `1.5px solid ${C.border}`, borderRadius: 8, fontSize: 14, boxSizing: "border-box", outline: "none", background: "#F8FAF9" }} />
);

const Select = ({ children, ...props }) => (
  <select {...props} style={{ width: "100%", padding: "10px 14px", border: `1.5px solid ${C.border}`, borderRadius: 8, fontSize: 14, background: "#F8FAF9" }}>
    {children}
  </select>
);
  ...
  return (
    <div>
      <div style={{ fontSize: 11, color: "#555", marginBottom: 8, padding: "6px 10px", background: "#fffbe6", border: "1px solid #f0d060", borderRadius: 6 }}>
        ...
        {box && <span style={{ marginLeft: 8, color: "#10B981", fontWeight: 600 }}>✓ Box defined: top {box.top}%, left {box.left}%, {box.width}% × {box.height}%</span>}
      </div>
      ...
      >
        ...
        {box && !dragging && (
          <div style={{
            ...
            <div style={{ position: "absolute", top: 2, left: 4, fontSize: 9, fontWeight: 700, color: "#10B981", background: "rgba(255,255,255,0.85)", padding: "1px 4px", borderRadius: 3, whiteSpace: "nowrap" }}>
              TEXT AREA
            </div>
          </div>
        )}
...
function CreditManagement({ companies }) {
  ...
  const GRAD = {
    navy:  "linear-gradient(135deg,#064E3B,#0A6B50)",
    green: "linear-gradient(135deg,#10B981,#0E9E6E)",
    warn:  "linear-gradient(135deg,#f39c12,#e67e22)",
    danger:"linear-gradient(135deg,#e74c3c,#c0392b)",
  };
  const C2 = { border: "#D6DCE5", muted: "#7A8F87", navy: "#064E3B", text: "#3D5A50" };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 2px 12px rgba(0,0,0,0.07)", padding: "22px 24px" }}>
        ...
        {assignMsg && <div style={{ marginTop: 10, fontSize: 13, color: assignMsg.startsWith("✓") ? "#10B981" : "#e74c3c" }}>{assignMsg}</div>}
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 2px 12px rgba(0,0,0,0.07)", overflow: "hidden" }}>
        <div style={{ padding: "16px 20px", borderBottom: `2px solid ${C2.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <button onClick={load} style={{ padding: "6px 14px", background: "#F3F6F5", border: "none", borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>↺ Refresh</button>
        </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr>{["Company", "Code", "Balance", "Low Alert", "Expiry", "Status", ""].map(h => (
                <th key={h} style={{ padding: "9px 14px", textAlign: "left", fontSize: 10, fontWeight: 700, color: C2.muted, textTransform: "uppercase", background: "#F8FAF9", borderBottom: `2px solid ${C2.border}` }}>{h}</th>
              ))}</tr>
            </thead>
            <tbody>
              {creditList.map((c, i) => {
                ...
                return (
                  <tr key={c.company_code} style={{ background: i % 2 === 0 ? "#fff" : "#F8FAF9" }}>
                    ...
                    <td style={{ padding: "9px 14px" }}>
                      <span style={{ background: critical ? "#fff5f5" : low ? "#fffbea" : "#E8F5EC", color: critical ? "#e74c3c" : low ? "#7a5c00" : "#0E9E6E", borderRadius: 6, padding: "3px 10px", fontWeight: 700, fontSize: 12 }}>
                        🪙 {bal}
                      </span>
                    </td>
                    ...
                    <td style={{ padding: "9px 14px" }}>
                      {expired ? <span style={{ background: "#fff5f5", color: "#e74c3c", borderRadius: 5, padding: "2px 8px", fontSize: 11, fontWeight: 700 }}>Expired</span>
                        ...
                        : <span style={{ background: "#E8F5EC", color: "#0E9E6E", borderRadius: 5, padding: "2px 8px", fontSize: 11, fontWeight: 700 }}>OK</span>}
                    </td>
                    <td style={{ padding: "9px 14px" }}>
                      <button onClick={() => loadHistory(c.company_code)}
                        style={{ padding: "4px 12px", background: selected === c.company_code ? GRAD.navy : "#F3F6F5", color: selected === c.company_code ? "#fff" : C2.text, border: "none", borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                        History
                      </button>
                    </td>
                  </tr>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr>{["Date & Time", "Actor", "Action", "Amount", "Before", "After", "Report", "Note"].map(h => (
                  <th key={h} style={{ padding: "8px 12px", textAlign: "left", fontSize: 10, fontWeight: 700, color: C2.muted, textTransform: "uppercase", background: "#F8FAF9", borderBottom: `2px solid ${C2.border}` }}>{h}</th>
                ))}</tr>
              </thead>
              <tbody>
                {history.map((tx, i) => (
                  <tr key={tx.id} style={{ background: i % 2 === 0 ? "#fff" : "#F8FAF9" }}>
                    ...
                    <td style={{ padding: "7px 12px" }}>
                      <span style={{ background: tx.action === "assign" ? "#E8F5EC" : "#fff5f5", color: tx.action === "assign" ? "#0E9E6E" : "#e74c3c", borderRadius: 5, padding: "2px 8px", fontSize: 11, fontWeight: 700 }}>
                        {tx.action === "assign" ? "＋ Assign" : "− Deduct"}
                      </span>
                    </td>
                    <td style={{ padding: "7px 12px", fontWeight: 700, color: tx.action === "assign" ? "#0E9E6E" : "#e74c3c" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14 }}>
        {[
          { label: "Credit Balance", value: `🪙 ${balance}`, grad: critical ? "linear-gradient(135deg,#e74c3c,#c0392b)" : low ? "linear-gradient(135deg,#f39c12,#e67e22)" : "linear-gradient(135deg,#10B981,#0E9E6E)" },
          { label: "Expiry", value: expiry ? expiry.slice(0,10) : "No expiry", grad: expired ? "linear-gradient(135deg,#e74c3c,#c0392b)" : "linear-gradient(135deg,#10B981,#0E9E6E)" },
          { label: "Status", value: expired ? "⚠ Expired" : critical ? "⚠ Critical" : low ? "⚠ Low" : "✓ Active", grad: "linear-gradient(135deg,#064E3B,#0A6B50)" },
        ...
        {history.length === 0 ? <div style={{ padding: 24, textAlign: "center", color: C2.muted }}>No transactions yet.</div> : (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead><tr>{["Date", "User", "Action", "Credits", "Balance", "Report", "Note"].map(h => (
              <th key={h} style={{ padding: "7px 12px", textAlign: "left", fontSize: 10, fontWeight: 700, color: C2.muted, textTransform: "uppercase", background: "#F8FAF9", borderBottom: `2px solid ${C2.border}` }}>{h}</th>
            ))}</tr></thead>
            <tbody>{history.map((tx, i) => (
              <tr key={tx.id} style={{ background: i % 2 === 0 ? "#fff" : "#F8FAF9" }}>
                ...
                <td style={{ padding: "6px 12px" }}>
                  <span style={{ background: tx.action === "assign" ? "#E8F5EC" : "#fff5f5", color: tx.action === "assign" ? "#0E9E6E" : "#e74c3c", borderRadius: 5, padding: "2px 7px", fontSize: 10, fontWeight: 700 }}>
                    {tx.action === "assign" ? "＋ Top-up" : "− Print"}
                  </span>
                </td>
                <td style={{ padding: "6px 12px", fontWeight: 700, color: tx.action === "assign" ? "#0E9E6E" : "#e74c3c" }}>{tx.action === "assign" ? "+" : "-"}{tx.amount}</td>
...
function RateMapAccessAdmin({ companies }) {
  ...
  const C2 = { border: "#D6DCE5", muted: "#7A8F87", navy: "#064E3B" };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          <span style={{ background: "#E8F5EC", color: "#0E9E6E", border: "1.5px solid #10B981", borderRadius: 20, padding: "5px 14px", fontSize: 12, fontWeight: 700 }}>
            {freeCount} / {list.length} companies free
          </span>
          <button onClick={() => bulkToggle(true)} disabled={bulkSaving}
            style={{ padding: "8px 16px", background: "linear-gradient(135deg,#10B981,#0E9E6E)", color: "#fff", border: "none", borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
            ✅ Enable Free for All
          </button>
          ...
          <button onClick={load} style={{ padding: "8px 14px", background: "#F3F6F5", border: "none", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>↺</button>
        </div>
      </div>

      {msg && <div style={{ padding: "10px 14px", background: msg.startsWith("✓") ? "#E8F5EC" : "#fff5f5", border: `1px solid ${msg.startsWith("✓") ? "#10B981" : "#e74c3c"}`, borderRadius: 9, fontSize: 13, color: msg.startsWith("✓") ? "#0E9E6E" : "#c0392b", fontWeight: 600 }}>{msg}</div>}
      ...
      <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 2px 12px rgba(0,0,0,0.07)", padding: "22px 26px" }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {durations.map((d, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, background: "#F3F6F5", borderRadius: 9, padding: "10px 14px", flex: 1 }}>
          ...
          <button onClick={addDuration} style={{ marginTop: 10, padding: "7px 16px", background: "#F3F6F5", border: `1.5px solid ${C2.border}`, borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", color: C2.navy }}>＋ Add Tier</button>
        </div>
        {settingsMsg && <div style={{ fontSize: 13, color: settingsMsg.startsWith("✓") ? "#0E9E6E" : "#c0392b", fontWeight: 600, marginBottom: 10 }}>{settingsMsg}</div>}
        <button onClick={saveSettings} disabled={settingsSaving}
          style={{ padding: "10px 24px", background: "linear-gradient(135deg,#064E3B,#0A6B50)", color: "#fff", border: "none", borderRadius: 9, fontWeight: 700, fontSize: 13, cursor: "pointer", opacity: settingsSaving ? 0.7 : 1 }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead>
            <tr>{["Company", "Code", "Access Type", "Action"].map(h => (
              <th key={h} style={{ padding: "9px 16px", textAlign: "left", fontSize: 10, fontWeight: 700, color: C2.muted, textTransform: "uppercase", background: "#F8FAF9", borderBottom: `2px solid ${C2.border}` }}>{h}</th>
            ))}</tr>
          </thead>
          <tbody>
            {list.map((c, i) => (
              <tr key={c.company_code} style={{ background: i % 2 === 0 ? "#fff" : "#F8FAF9" }}>
                ...
                <td style={{ padding: "10px 16px" }}>
                  {c.rate_map_free
                    ? <span style={{ background: "#E8F5EC", color: "#0E9E6E", borderRadius: 6, padding: "3px 10px", fontSize: 12, fontWeight: 700 }}>✅ Free Access</span>
                    : <span style={{ background: "#F3F6F5", color: C2.muted, borderRadius: 6, padding: "3px 10px", fontSize: 12, fontWeight: 700 }}>🪙 Credit-based</span>}
                </td>
                <td style={{ padding: "10px 16px" }}>
                  <button onClick={() => toggle(c.company_code, !!c.rate_map_free)} disabled={!!saving[c.company_code]}
                    style={{ padding: "6px 16px", border: "none", borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: "pointer", background: c.rate_map_free ? "linear-gradient(135deg,#e74c3c,#c0392b)" : "linear-gradient(135deg,#10B981,#0E9E6E)", color: "#fff", opacity: saving[c.company_code] ? 0.6 : 1 }}>
...
function MiniBar({ pct, color }) {
  return (
    <div style={{ background: "#D6DCE5", borderRadius: 4, height: 6, width: "100%", overflow: "hidden" }}>
...
function StoragePanel() {
  ...
  const barColors = ["#10B981", "#8e44ad", "#10B981", "#f39c12"];
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            ...
            <tbody>
              {data.companies.map((co, i) => {
                ...
                return (
                  <tr key={co.company_code} style={{ borderBottom: `1px solid ${C.border}`, background: i % 2 === 0 ? "#fff" : "#F8FAF9" }}>
                    ...
                    <td style={{ padding: "10px 14px", color: "#10B981" }}>{fmtBytes(bd.reports || 0)}</td>
                    <td style={{ padding: "10px 14px", color: "#8e44ad" }}>{fmtBytes(bd.versions || 0)}</td>
                    <td style={{ padding: "10px 14px", color: "#10B981" }}>{fmtBytes(bd.letterhead || 0)}</td>
                    <td style={{ padding: "10px 14px", color: "#f39c12" }}>{fmtBytes(bd.field || 0)}</td>
                    <td style={{ padding: "10px 14px", minWidth: 120 }}><MiniBar pct={pct} color="#10B981" /><span style={{ fontSize: 10, color: C.muted }}>{pct.toFixed(1)}%</span></td>
                  </tr>
                );
              })}
            </tbody>
...
function BroadcastEmail({ companies }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      const html = `<div style="font-family:'Segoe UI',Arial,sans-serif;max-width:580px;margin:0 auto;background:#fff;border-radius:12px;border:1px solid #D6DCE5;overflow:hidden">
        <div style="background:linear-gradient(135deg,#064E3B,#0A6B50);padding:28px 32px;color:#fff">
          <h1 style="margin:0;font-size:20px;font-weight:800">${subject}</h1>
          <p style="margin:6px 0 0;opacity:0.7;font-size:12px">Mulyanka — One Degree Consultant Pvt. Ltd.</p>
        </div>
        <div style="padding:28px 32px;font-size:14px;color:#3D5A50;line-height:1.8;white-space:pre-wrap">${body.replace(/</g,"&lt;").replace(/>/g,"&gt;")}</div>
        <div style="padding:16px 32px;border-top:1px solid #D6DCE5;font-size:12px;color:#aaa">
  ...
  return (
    <div style={{ maxWidth: 720 }}>
      ...
      {/* ── Test Email Card ── */}
      <div style={{ background: "#fff", borderRadius: 12, border: `1.5px solid #6FD28C`, padding: "20px 24px", marginBottom: 20 }}>
        <h3 style={{ margin: "0 0 4px", fontSize: 15, fontWeight: 800, color: "#0E9E6E" }}>🧪 Test Email Configuration</h3>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button onClick={sendTest} disabled={testSending}
            style={{ padding: "9px 20px", background: "#0E9E6E", color: "#fff", border: "none", borderRadius: 8, fontWeight: 700, fontSize: 13, cursor: testSending ? "not-allowed" : "pointer", whiteSpace: "nowrap", opacity: testSending ? 0.7 : 1 }}>
            {testSending ? "Sending…" : "Send Test"}
          </button>
        </div>
        {testResult && (
          <div style={{ marginTop: 10, padding: "8px 14px", borderRadius: 7, background: testResult.ok ? "#E8F5EC" : "#fff5f5", border: `1px solid ${testResult.ok ? "#6FD28C" : "#fca5a5"}`, color: testResult.ok ? "#0E9E6E" : "#c0392b", fontSize: 13, fontWeight: 600 }}>
            {testResult.msg}
          </div>
        )}
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C2.border}`, padding: "24px 28px", marginBottom: 20 }}>
        ...
        {result && (
          <div style={{ marginTop: 14, padding: "10px 16px", borderRadius: 8, background: result.ok ? "#E8F5EC" : "#fff5f5", border: `1px solid ${result.ok ? "#6FD28C" : "#fca5a5"}`, color: result.ok ? "#0E9E6E" : "#c0392b", fontSize: 13, fontWeight: 600 }}>
            {result.msg}
          </div>
        )}
      </div>
...
function RegistrationsPanel({ onCountChange }) {
  ...
  const statusBg    = { pending: "#fffbea", approved: "#E8F5EC", rejected: "#fff5f5" };
          ...
          {["pending", "approved", "rejected", ""].map((f) => (
            <button key={f || "all"} onClick={() => { setFilter(f); load(f); }}
              style={{ padding: "6px 14px", border: "none", borderRadius: 7, cursor: "pointer", fontSize: 12, fontWeight: 700,
                background: filter === f ? GRAD.blue : "#F3F6F5",
                color: filter === f ? "#fff" : C.muted,
                boxShadow: filter === f ? "0 2px 8px rgba(16, 185, 129,0.3)" : "none" }}>
              {f || "All"}
            </button>
          ))}
          <button onClick={() => load(filter)} style={{ padding: "6px 12px", background: "#F3F6F5", border: "none", borderRadius: 7, cursor: "pointer", fontSize: 13 }}>↺</button>
        </div>
      </div>

      {msg && (
        <div style={{ padding: "10px 16px", background: msg.startsWith("✓") ? "#E8F5EC" : "#fff5f5",
          border: `1px solid ${msg.startsWith("✓") ? C.success : C.danger}`,
          borderRadius: 9, fontSize: 13, color: msg.startsWith("✓") ? "#0E9E6E" : C.danger, fontWeight: 600 }}>
          {msg}
        </div>
      )}
      ...
      <div style={S.section}>
        {loading ? (
          ...
          <table style={S.table}>
            ...
            <tbody>
              {requests.map((r, i) => (
                <tr key={r.id} style={{ background: i % 2 === 0 ? "#fff" : "#F8FAF9" }}>
    ...
    <div style={S.page}>
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                <div style={{ position: "relative" }}>
                  ...
                  <input
                    ...
                    style={{ padding: "8px 32px 8px 34px", border: `1.5px solid ${C.border}`, borderRadius: 8, fontSize: 13, width: 210, outline: "none", background: "#F8FAF9" }}
                  />
                  {companySearch && (
            ...
            <table style={S.table}>
              ...
              <tbody>
                {filteredCompanies.map((c, i) => {
                  ...
                  const creditBg     = expired || critical ? "#fff5f5"  : low ? "#fffbea" : "#E8F5EC";
                  return (
                    <tr key={c.id} style={{ background: i % 2 === 0 ? "#fff" : "#F8FAF9" }}>
                      <td style={S.td}>
                        <code style={{ background: "#E8F5EC", color: C.blue, padding: "2px 8px", borderRadius: 5, fontSize: 12, fontWeight: 700 }}>{c.company_code}</code>
                      </td>
                      <td style={S.td}>
                        <span
          ...
          <div style={{ ...S.section, border: `1.5px solid ${C.navy}20`, marginBottom: 18 }}>
            ...
            {systemUsers.length === 0 ? (
              ...
              <table style={S.table}>
                ...
                <tbody>
                  {systemUsers.map((u, i) => (
                    <tr key={u.id} style={{ background: i % 2 === 0 ? "#fff" : "#F8FAF9" }}>
              ...
              return (
                <div key={group.company_code}>
                  ...
                  <div onClick={() => toggleCompany(group.company_code)} style={{
                    ...
                    background: isOpen ? "#E8F5EC" : "#fff",
                    borderBottom: `1px solid ${C.border}`,
                    transition: "background 0.15s",
                  }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <div style={{ width: 28, height: 28, borderRadius: 7, background: isOpen ? GRAD.blue : "#F3F6F5", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, color: isOpen ? "#fff" : C.muted, transition: "all 0.15s" }}>
                        {isOpen ? "▼" : "▶"}
                      </div>
                      <div>
                        <span style={{ fontWeight: 700, fontSize: 14, color: isOpen ? C.blue : C.navy }}>{group.company_name || group.company_code}</span>
                        <code style={{ marginLeft: 9, background: "#E8F5EC", color: C.muted, padding: "1px 7px", borderRadius: 4, fontSize: 11 }}>{group.company_code}</code>
                      </div>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  {/* Expanded */}
                  {isOpen && (
                    <div style={{ background: "#F8FAF9", borderBottom: `2px solid #1a73e820` }}>
                    ...
                    {stats.perCompany.length === 0 ? (
                      ...
                      return (
                        <React.Fragment key={row.company_code}>
                          ...
                          <tr
                            style={{ background: isOpen ? "#E8F5EC" : i % 2 === 0 ? "#fff" : "#F8FAF9", cursor: "pointer", transition: "background 0.15s" }}
                            onClick={() => {
                              if (isOpen) {
                          ...
                          >
                            <td style={S.td}><code style={{ background: "#E8F5EC", color: C.blue, padding: "2px 8px", borderRadius: 5, fontSize: 12, fontWeight: 700 }}>{row.company_code || "—"}</code></td>
                            ...
                            <td style={S.td}><span style={{ ...S.tag("#0E9E6E"), padding: "3px 10px", fontSize: 12 }}>{row.preliminary}</span></td>
                          ...
                          {isOpen && (
                            <tr>
                              <td colSpan={7} style={{ padding: 0, background: "#E8F5EC", borderBottom: `2px solid ${C.blue}33` }}>
                                {companyReportsLoading ? (
                                  ...
                                  <div style={{ overflowX: "auto" }}>
                                      ...
                                      <thead>
                                        <tr style={{ background: "#E8F5EC" }}>
                                          {["#", "Client / Owner", "Bank", "Branch", "Type", "Visit Date", "Report Date", "Created By", "Last Saved"].map(h => (
                                            <th key={h} style={{ ...S.th, background: "#E8F5EC", fontSize: 10, padding: "8px 12px" }}>{h}</th>
                                          ))}
                                        </tr>
                                      </thead>
                                      <tbody>
                                        {companyReports.map((r, ri) => (
                                          <tr key={r.id} style={{ background: ri % 2 === 0 ? "#fff" : "#F8FAF9" }}>
                                            ...
                                            <td style={{ ...S.td, padding: "8px 12px" }}>
                                              <span style={{
                                                background: r.report_type === "preliminary" ? "#E8F5EC" : "#E8F5EC",
                                                color: r.report_type === "preliminary" ? "#0E9E6E" : C.success,
                                    ...
                                    <div style={{ padding: "8px 16px", fontSize: 11, color: C.muted, background: "#E8F5EC", borderTop: `1px solid ${C.border}` }}>
                ...
                <table style={S.table}>
                  ...
                  <tbody>
                    {stats.recent.length === 0 ? (
                      ...
                      <tr key={r.id} style={{ background: i % 2 === 0 ? "#fff" : "#F8FAF9" }}>
                        <td style={{ ...S.td, color: C.muted, fontSize: 11 }}>{r.id}</td>
                        <td style={S.td}><code style={{ background: "#E8F5EC", color: C.blue, padding: "1px 6px", borderRadius: 4, fontSize: 11 }}>{r.company_code || "—"}</code></td>
                        <td style={{ ...S.td, fontWeight: 500 }}>{r.client_name || "—"}</td>
                        <td style={S.td}>{r.bank || "—"}</td>
                        <td style={S.td}>{r.branch || "—"}</td>
                        <td style={S.td}><span style={{ ...S.tag(r.report_type === "preliminary" ? "#0E9E6E" : C.success), fontSize: 11 }}>{r.report_type}</span></td>
      ...
      {detailCompany && (() => {
        ...
        const creditStatus = expired ? { label: "Expired", color: C.danger, bg: "#fff5f5" }
          ...
          :             { label: "Healthy",  color: C.success, bg: "#E8F5EC" };
        ...
        return (
          <>
            ...
            <div onClick={() => setDetailCompany(null)}
              style={{ position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.45)", zIndex: 8000, backdropFilter: "blur(2px)" }} />
              ...
              <div style={{ padding: "24px 28px", display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                <div style={{ background: creditStatus.bg, border: `1.5px solid ${creditStatus.color}33`, borderRadius: 12, padding: "18px 20px" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "flex-end", gap: 12, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 48, fontWeight: 900, color: "#064E3B", lineHeight: 1 }}>{bal}</span>
                ...
                {/* Contact info */}
                <div style={{ background: "#F8FAF9", borderRadius: 12, padding: "18px 20px" }}>
                ...
                {/* Users */}
                <div style={{ background: "#F8FAF9", borderRadius: 12, overflow: "hidden" }}>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                      <thead>
                        <tr>{["Username","Role","Status"].map(h => (
                          <th key={h} style={{ padding: "8px 16px", textAlign: "left", fontSize: 10, fontWeight: 700, color: C.muted, textTransform: "uppercase", background: "#F3F6F5", borderBottom: `1px solid ${C.border}` }}>{h}</th>
                        ))}</tr>
                      </thead>
                      <tbody>
                        {members.map((u, i) => (
          ...
          <form onSubmit={saveCompany}>
            ...
            <div style={{ marginTop: 18, borderTop: `1px solid ${C.border}`, paddingTop: 16 }}>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 12 }}>
                <label style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "9px 18px", background: "#E8F5EC", border: `1.5px solid ${C.blue}`, color: C.blue, borderRadius: 8, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                  📎 Choose PNG / JPG
