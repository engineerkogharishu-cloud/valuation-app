...
export function CreditBadge() {
  ...
  return (
    <div title={`Credit balance: ${balance}`} style={{
      ...
      background: critical ? "rgba(231,76,60,0.2)" : low ? "rgba(243,156,18,0.2)" : "rgba(16, 185, 129,0.15)",
      border: `1px solid ${critical ? "rgba(231,76,60,0.5)" : low ? "rgba(243,156,18,0.5)" : "rgba(16, 185, 129,0.4)"}`,
      borderRadius: 20, padding: "3px 10px", fontSize: 12, fontWeight: 700,
      color: critical ? "#e74c3c" : low ? "#f39c12" : "#10B981",
    }}>
...
function PrintConfirmModal({ type, cost, balance, reportId, onProceed, onClose, onSuccess }) {
  ...
  return (
    <div style={{
      position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.65)",
      ...
      <div style={{
        ...
        fontFamily: "'Poppins', system-ui, -apple-system, 'Poppins', system-ui, -apple-system, 'Segoe UI', sans-serif",
      }}>
        {/* Header */}
        <div style={{
          background: canPrint ? "linear-gradient(135deg,#064E3B,#0A6B50)" : "linear-gradient(135deg,#c0392b,#e74c3c)",
        ...
        <div style={{ padding: "20px 24px" }}>
          {canPrint ? (
            <>
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
                {[
                  ...
                  <div key={label} style={{
                    ...
                    padding: "9px 14px", background: "#F3F6F5", borderRadius: 8,
                  }}>
                    <span style={{ fontSize: 13, color: "#7A8F87" }}>{label}</span>
                    <span style={{ fontSize: 13, fontWeight: 700, color: remaining <= 2 && label === "Balance After" ? "#e74c3c" : "#064E3B" }}>
                      {value}
                    </span>
                  </div>
                ))}
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={onClose} disabled={loading} style={{
                  flex: 1, padding: "10px", border: "1.5px solid #D6DCE5", borderRadius: 8,
                  background: "#fff", color: "#3D5A50", fontWeight: 600, fontSize: 13, cursor: "pointer",
                }}>Cancel</button>
                <button onClick={handleConfirm} disabled={loading} style={{
                  flex: 2, padding: "10px", border: "none", borderRadius: 8,
                  background: "linear-gradient(135deg,#064E3B,#0A6B50)", color: "#fff",
            ...
            <>
              <p style={{ fontSize: 14, color: "#7A8F87", lineHeight: 1.6, marginBottom: 20 }}>
              ...
              <button onClick={onClose} style={{
                width: "100%", padding: "10px", border: "1.5px solid #D6DCE5", borderRadius: 8,
                background: "#fff", color: "#3D5A50", fontWeight: 600, fontSize: 13, cursor: "pointer",
              }}>Close</button>
            </>
          )}
