...
export default function PhotoEditor({ photo, onSave, onClose }) {
  ...
  const drawAnnotation = (ctx, a) => {
    ...
    if (a.type === "rect") {
      ...
      ctx.font = `bold ${a.fontSize || 16}px 'Poppins', sans-serif`;
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", zIndex:99999,
      ...
      {/* Toolbar */}
      <div style={{ background:"#04382A", padding:"10px 16px", display:"flex",
          ...
          {TOOLS.map(t => (
            <button key={t.id} onClick={() => setTool(t.id)} title={t.label}
              style={{ padding:"6px 12px", borderRadius:"7px", border:"none", cursor:"pointer",
                ...
                background: tool === t.id ? "#E0C468" : "rgba(255,255,255,0.12)",
                color: tool === t.id ? "#04382A" : "#fff" }}>
              {t.icon} {t.label}
            </button>
          ))}
        </div>
              ...
              {[
                ...
                <button key={s.id} onClick={()=>setArrowSize(s.id)}
                  ...
                  style={{ padding:"4px 10px", borderRadius:6, border:"none", cursor:"pointer",
                    ...
                    background: arrowSize===s.id ? "#E0C468" : "rgba(255,255,255,0.12)",
                    color: arrowSize===s.id ? "#04382A" : "#fff" }}>
        ...
        <div style={{ marginLeft:"auto", display:"flex", gap:"8px" }}>
          ...
          <button onClick={save}
            style={{ padding:"7px 20px", borderRadius:8, border:"none",
              background:"linear-gradient(135deg,#10B981,#0E9E6E)", color:"#fff",
      ...
      <div style={{ background:"#111", padding:"6px 16px", fontSize:"11px",
        ...
        <span>Tool: <strong style={{color:"#E0C468"}}>{TOOLS.find(t=>t.id===tool)?.label}</strong></span>
        {tool === "arrow" && <span>Arrow: <strong style={{color:"#E0C468"}}>{arrowSize}</strong></span>}
