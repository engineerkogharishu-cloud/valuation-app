...
function FeedbackDetailModal({ feedbackId, onClose, onApprove, onReject }) {
  ...
  const STATUS_META = {
    ...
    approved: { label: "Approved", bg: "#E8F5EC", color: "#0E9E6E" },
  ...
  return createPortal(
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.6)", backdropFilter: "blur(3px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 99999, padding: 16 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 640, maxHeight: "90vh", overflow: "auto", boxShadow: "0 24px 64px rgba(0,0,0,0.35)", fontFamily: "'Poppins', system-ui, -apple-system, 'Poppins', system-ui, -apple-system, 'Segoe UI', sans-serif" }}>
        {/* Header */}
        <div style={{ background: "linear-gradient(135deg,#064E3B,#0A6B50)", padding: "20px 26px", color: "#fff", display: "flex", justifyContent: "space-between", alignItems: "flex-start", position: "sticky", top: 0, zIndex: 1 }}>
          <div>
            ...
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 18 }}>
                {[
                  ...
                  ["Credit Awarded", fb.credit_awarded ? <span style={{ color: "#0E9E6E", fontWeight: 700 }}>✅ +1 🪙</span> : "—"],
                  ...
                  <div key={label} style={{ background: "#F3F6F5", borderRadius: 8, padding: "10px 14px" }}>
              ...
              <div style={{ marginBottom: 18 }}>
                ...
                <div style={{ background: "#F3F6F5", borderRadius: 10, padding: "14px 16px", fontSize: 14, color: "#3D5A50", lineHeight: 1.8, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>{fb.message}</div>
              </div>
              ...
              {fb.screenshot ? (
                ...
                <div style={{ marginBottom: 18, padding: "12px 16px", background: "#F3F6F5", borderRadius: 9, fontSize: 13, color: C.muted }}>
                  ...
                  {!showReject ? (
                    <div style={{ display: "flex", gap: 10 }}>
                      <button onClick={handleApprove} disabled={actioning === "approve"}
                        style={{ flex: 2, padding: "11px", border: "none", borderRadius: 9, background: "linear-gradient(135deg,#10B981,#0E9E6E)", color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer", opacity: actioning ? 0.7 : 1 }}>
...
const STATUS_META = {
  ...
  approved: { label: "Approved", bg: "#E8F5EC", color: "#0E9E6E", border: "#10B981" },
...
function RejectModal({ feedback, onConfirm, onClose }) {
  ...
  return createPortal(
    <div style={{ position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.55)", backdropFilter: "blur(3px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 99999, padding: 16 }}>
      <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 440, boxShadow: "0 24px 64px rgba(0,0,0,0.3)", overflow: "hidden", fontFamily: "'Poppins', system-ui, -apple-system, 'Poppins', system-ui, -apple-system, 'Segoe UI', sans-serif" }}>
        ...
        <div style={{ padding: "20px 24px" }}>
          <div style={{ background: "#F3F6F5", borderRadius: 9, padding: "10px 14px", fontSize: 13, color: "#3D5A50", lineHeight: 1.6, marginBottom: 16, fontStyle: "italic" }}>
            "{feedback.message}"
          </div>
          <label style={{ fontSize: 12, fontWeight: 700, color: "#7A8F87", textTransform: "uppercase", display: "block", marginBottom: 6 }}>Rejection Note (optional)</label>
          <textarea value={note} onChange={e => setNote(e.target.value)} rows={3} placeholder="Reason for rejection…"
            style={{ width: "100%", padding: "10px 12px", border: "1.5px solid #D6DCE5", borderRadius: 8, fontSize: 13, resize: "vertical", fontFamily: "inherit", boxSizing: "border-box", marginBottom: 16 }} />
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={onClose} disabled={loading} style={{ flex: 1, padding: "10px", border: "1.5px solid #D6DCE5", borderRadius: 8, background: "#fff", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>Cancel</button>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
        ...
        <button onClick={load} disabled={loading} style={{ padding: "8px 16px", background: "#F3F6F5", border: "none", borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: "pointer" }}>↺ Refresh</button>
      </div>
      ...
      {stats && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 12 }}>
          {[
            ...
            { label: "Credited 🪙", value: stats.credited, grad: "linear-gradient(135deg,#10B981,#0E9E6E)" },
              ...
              {[{ company_code: "", company_name: "All Companies" }, ...companies]
                ...
                .map(c => (
                  <div key={c.company_code}
                    ...
                    style={{ padding: "9px 14px", cursor: "pointer", fontSize: 13, background: filterCompany === c.company_code ? "#E8F5EC" : "transparent", color: filterCompany === c.company_code ? "#10B981" : "#3D5A50", fontWeight: filterCompany === c.company_code ? 700 : 400 }}
                    onMouseEnter={e => e.currentTarget.style.background = filterCompany === c.company_code ? "#E8F5EC" : "#F3F6F5"}
                    onMouseLeave={e => e.currentTarget.style.background = filterCompany === c.company_code ? "#E8F5EC" : "transparent"}
                  >
                    {c.company_name}{c.company_code ? <span style={{ fontSize: 11, color: "#7A8F87", marginLeft: 6 }}>({c.company_code})</span> : null}
                  </div>
                ))}
            </div>
          )}
      ...
      {actionMsg && (
        <div style={{ padding: "11px 16px", background: actionMsg.startsWith("✓") ? "#E8F5EC" : actionMsg.startsWith("⚠") ? "#fff5f5" : "#F3F6F5", border: `1px solid ${actionMsg.startsWith("✓") ? "#10B981" : actionMsg.startsWith("⚠") ? "#e74c3c" : "#D6DCE5"}`, borderRadius: 9, fontSize: 13, color: actionMsg.startsWith("✓") ? "#0E9E6E" : actionMsg.startsWith("⚠") ? "#c0392b" : "#3D5A50", fontWeight: 600 }}>
          {actionMsg}
        </div>
      )}
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr>{["#", "User", "Company", "Feedback", "📎", "Status", "Credit", "Email", "Approved By", "Date", "Actions"].map(h => (
                <th key={h} style={{ padding: "9px 13px", textAlign: "left", fontSize: 10, fontWeight: 700, color: C.muted, textTransform: "uppercase", background: "#F8FAF9", borderBottom: `2px solid ${C.border}`, whiteSpace: "nowrap" }}>{h}</th>
              ))}</tr>
            </thead>
            <tbody>
              {feedback.map((f, i) => (
                <tr key={f.id} style={{ background: i % 2 === 0 ? "#fff" : "#F8FAF9", verticalAlign: "top" }}>
                  ...
                  <td style={{ padding: "10px 13px", maxWidth: 240 }}>
                    <div style={{ color: "#3D5A50", lineHeight: 1.6, wordBreak: "break-word", display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{f.message}</div>
                    {f.rejection_note && (
                  ...
                  <td style={{ padding: "10px 13px", whiteSpace: "nowrap" }}>
                    {f.credit_awarded
                      ? <span style={{ background: "#E8F5EC", color: "#0E9E6E", borderRadius: 5, padding: "2px 8px", fontSize: 11, fontWeight: 700 }}>✅ +1</span>
                      : <span style={{ color: C.muted }}>—</span>}
                  </td>
                  <td style={{ padding: "10px 13px" }}>
                    {f.email_sent
                      ? <span title="Thank-you email sent" style={{ color: "#10B981", fontSize: 15 }}>✉✓</span>
                      : <span style={{ color: C.muted }}>—</span>}
                  </td>
                  ...
                  <td style={{ padding: "10px 13px" }}>
                    <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                      <button onClick={() => setViewTarget(f.id)}
                        style={{ padding: "5px 13px", background: "linear-gradient(135deg,#10B981,#0E9E6E)", color: "#fff", border: "none", borderRadius: 6, fontWeight: 700, fontSize: 11, cursor: "pointer", whiteSpace: "nowrap" }}>
                        👁 View
                      </button>
                      {f.status === "pending" && (
                        <>
