...
export default function FeedbackSection({ user }) {
  ...
  const C = { navy: "#064E3B", border: "#D6DCE5", muted: "#7A8F87", bg: "#F3F6F5" };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {feedback.length > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
          {[
            { label: "Submitted",     value: feedback.length, color: "#10B981", bg: "#E8F5EC" },
            { label: "Approved",      value: approved,        color: "#0E9E6E", bg: "#E8F5EC" },
      ...
      <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 2px 12px rgba(0,0,0,0.07)", padding: "22px 26px" }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <textarea
            ...
            rows={5}
            maxLength={2000}
            style={{ padding: "12px 14px", border: `1.5px solid ${C.border}`, borderRadius: 10, fontSize: 14, resize: "vertical", fontFamily: "'Poppins', system-ui, -apple-system, 'Segoe UI', sans-serif", lineHeight: 1.6 }}
          />
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              {submitMsg && <span style={{ fontSize: 13, color: submitMsg.startsWith("✓") ? "#0E9E6E" : "#c0392b", fontWeight: 600 }}>{submitMsg}</span>}
              <button type="submit" disabled={submitting || !message.trim()}
                style={{ padding: "10px 24px", background: message.trim() ? "linear-gradient(135deg,#064E3B,#0A6B50)" : "#ccc", color: "#fff", border: "none", borderRadius: 9, fontWeight: 700, fontSize: 14, cursor: message.trim() && !submitting ? "pointer" : "not-allowed", opacity: submitting ? 0.7 : 1 }}>
                {submitting ? "Submitting…" : "📤 Submit Feedback"}
              </button>
            </div>
          </div>
        </form>
        <div style={{ marginTop: 14, background: "#E8F5EC", border: "1px solid #10B981", borderRadius: 9, padding: "10px 14px", fontSize: 13, color: "#0E9E6E", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr>{(isAdmin ? ["#", "By", "Feedback", "📎", "Status", "Credit", "Approved By", "Date"] : ["#", "Feedback", "📎", "Status", "Credit", "Approved By", "Date"]).map(h => (
                <th key={h} style={{ padding: "9px 14px", textAlign: "left", fontSize: 10, fontWeight: 700, color: C.muted, textTransform: "uppercase", background: "#F8FAF9", borderBottom: `2px solid ${C.border}` }}>{h}</th>
              ))}</tr>
            </thead>
            <tbody>
              {feedback.map((f, i) => (
                <tr key={f.id} style={{ background: i % 2 === 0 ? "#fff" : "#F8FAF9", verticalAlign: "top" }}>
                  ...
                  <td style={{ padding: "10px 14px", maxWidth: 340 }}>
                    <div style={{ color: "#3D5A50", lineHeight: 1.6, wordBreak: "break-word" }}>{f.message}</div>
                  ...
                  <td style={{ padding: "10px 14px" }}>
                    {f.has_screenshot
                      ? <button onClick={async () => { const d = await api.getFeedbackDetail(f.id); setLightbox(d.screenshot); }}
                          style={{ padding: "4px 10px", background: "#E8F5EC", color: "#10B981", border: "none", borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>🖼 View</button>
                      : <span style={{ color: C.muted }}>—</span>}
                  </td>
                  <td style={{ padding: "10px 14px", whiteSpace: "nowrap" }}><StatusBadge status={f.status} /></td>
                  <td style={{ padding: "10px 14px", whiteSpace: "nowrap" }}>
                    {f.credit_awarded
                      ? <span style={{ background: "#E8F5EC", color: "#0E9E6E", borderRadius: 6, padding: "2px 9px", fontSize: 11, fontWeight: 700 }}>✅ +1 🪙</span>
                      : <span style={{ color: C.muted }}>—</span>}
                  </td>
