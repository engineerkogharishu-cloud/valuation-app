...
const GROUP_META = {
  client:   { label: "Client",   color: "#064E3B", bg: "#E8F1EC" },
  owner:    { label: "Owner",    color: "#6d28d9", bg: "#ede9fe" },
  land:     { label: "Land",     color: "#0E9E6E", bg: "#E8F5EC" },  // brand green
...
const DOC_STATUSES = [
  { value: "available",    label: "✓ Available",    bg: "#10B981", border: "#10B981", textBg: "#E8F5EC", textColor: "#0E9E6E" },
  { value: "not_required", label: "Not Required",   bg: "#7A8F87", border: "#7A8F87", textBg: "#f5f5f5", textColor: "#444" },
...
function MissingDocumentPanel({ clients, owners, hasBuilding, properties, docChecks, setDocChecks, customDocLabels, setCustomDocLabels }) {
  ...
  return (
    <div style={{ background: "#fff", border: "1.5px solid #D6DCE5", borderRadius: 12, overflow: "hidden" }}>
      <div
        ...
        <div style={{ padding: "16px 18px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input
              ...
              style={{ flex: 1, padding: "9px 12px", border: "1.5px solid #D6DCE5", borderRadius: 8, fontSize: 13, boxSizing: "border-box" }}
            />
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.65)", backdropFilter: "blur(3px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9500 }}>
      <div style={{ background: "#fff", borderRadius: 16, padding: "30px 32px", width: "96%", maxWidth: 480, boxShadow: "0 24px 64px rgba(0,0,0,0.3)" }}>
        <h3 style={{ margin: "0 0 6px", fontSize: 17, fontWeight: 700, color: "#064E3B" }}>📋 Field Charge</h3>
        <p style={{ margin: "0 0 22px", fontSize: 13, color: "#7A8F87" }}>Before generating the Preliminary Report, confirm field charge details.</p>

        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#064E3B", marginBottom: 10 }}>Did you receive field charge from the client?</div>
          <div style={{ display: "flex", gap: 10 }}>
            {[{ val: true, label: "✅ Yes, Received" }, { val: false, label: "❌ No, Not Received" }].map(({ val, label }) => (
              <button key={String(val)} onClick={() => setReceived(val)}
                style={{ flex: 1, padding: "11px 0", borderRadius: 9, border: `2px solid ${received === val ? (val ? "#10B981" : "#e74c3c") : "#D6DCE5"}`, background: received === val ? (val ? "#E8F5EC" : "#fff5f5") : "#F8FAF9", color: received === val ? (val ? "#0E9E6E" : "#c0392b") : "#555", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                {label}
              </button>
            ))}
          </div>
        ...
        {received && (
          <>
            <div style={{ marginBottom: 14 }}>
              <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "#7A8F87", textTransform: "uppercase", marginBottom: 5 }}>Field Charge Amount (NPR)</label>
              <input type="number" value={amount} onChange={e => setAmount(e.target.value)} min="0" placeholder="e.g. 2000"
                style={{ width: "100%", padding: "9px 12px", border: "1.5px solid #D6DCE5", borderRadius: 8, fontSize: 14, boxSizing: "border-box" }} />
            </div>
            <div style={{ marginBottom: 14 }}>
              <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "#7A8F87", textTransform: "uppercase", marginBottom: 5 }}>Transportation Charge (NPR) <span style={{ fontWeight: 400, textTransform: "none" }}>(optional)</span></label>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 22, justifyContent: "flex-end" }}>
          <button onClick={onCancel} style={{ padding: "9px 20px", border: "1.5px solid #D6DCE5", borderRadius: 8, background: "#fff", color: "#555", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>Cancel</button>
          <button onClick={() => canProceed && onConfirm(received, amount, transport)} disabled={!canProceed}
            style={{ padding: "9px 22px", background: canProceed ? "linear-gradient(135deg,#064E3B,#0A6B50)" : "#ccc", color: "#fff", border: "none", borderRadius: 8, fontWeight: 700, fontSize: 13, cursor: canProceed ? "pointer" : "not-allowed" }}>
            Generate Report →
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.65)", backdropFilter: "blur(3px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9500 }}>
      <div style={{ background: "#fff", borderRadius: 16, padding: "30px 32px", width: "96%", maxWidth: 520, boxShadow: "0 24px 64px rgba(0,0,0,0.3)" }}>
        <h3 style={{ margin: "0 0 6px", fontSize: 17, fontWeight: 700, color: "#064E3B" }}>⚠️ Verify Limiting Conditions</h3>
        <p style={{ margin: "0 0 18px", fontSize: 13, color: "#7A8F87" }}>Before generating the Final Report, confirm that all limiting conditions have been fulfilled.</p>
        ...
        <label style={{ display: "flex", alignItems: "flex-start", gap: 10, cursor: "pointer", marginBottom: 24 }}>
          <input
            ...
            style={{ width: 18, height: 18, accentColor: "#0A6B50", marginTop: 2, flexShrink: 0, cursor: "pointer" }}
          />
          <span style={{ fontSize: 13, color: "#064E3B", fontWeight: 500, lineHeight: 1.5 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onCancel}
            style={{ flex: 1, padding: "11px 0", borderRadius: 9, border: "2px solid #D6DCE5", background: "#F8FAF9", color: "#555", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
            ✗ Cancel — Go Back
          </button>
          <button onClick={onConfirm} disabled={!checked}
            style={{ flex: 1, padding: "11px 0", borderRadius: 9, border: "none", background: checked ? "linear-gradient(135deg,#0E9E6E,#10B981)" : "#ccc", color: "#fff", fontWeight: 700, fontSize: 13, cursor: checked ? "pointer" : "not-allowed" }}>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.65)", backdropFilter: "blur(3px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9500 }}>
      <div style={{ background: "#fff", borderRadius: 16, padding: "30px 32px", width: "96%", maxWidth: 460, boxShadow: "0 24px 64px rgba(0,0,0,0.3)" }}>
        <h3 style={{ margin: "0 0 6px", fontSize: 17, fontWeight: 700, color: "#064E3B" }}>💰 Payment Received</h3>
        <p style={{ margin: "0 0 22px", fontSize: 13, color: "#7A8F87" }}>Record how much has been received from the client before generating the Final Report.</p>

        <div style={{ marginBottom: 16 }}>
          <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "#7A8F87", textTransform: "uppercase", marginBottom: 8 }}>
          ...
          <input
            ...
            autoFocus
            style={{ width: "100%", padding: "11px 14px", border: "1.5px solid #D6DCE5", borderRadius: 9, fontSize: 15, boxSizing: "border-box" }}
          />
          <div style={{ fontSize: 11, color: canGenerate ? "#aaa" : "#e67e22", marginTop: 5, fontWeight: canGenerate ? 400 : 600 }}>
            {canGenerate ? "Amount received from the client." : "⚠ Payment must be received to generate the Final Report."}
          </div>
        </div>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <input
            ...
            style={{ width: "100%", padding: "11px 14px", border: "1.5px solid #D6DCE5", borderRadius: 9, fontSize: 14, boxSizing: "border-box" }}
          />
        </div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button onClick={() => canGenerate && onConfirm(amount, receivedAt, receivedBank)} disabled={!canGenerate}
            style={{ flex: 1, padding: "11px 0", borderRadius: 9, border: "none", background: canGenerate ? "linear-gradient(135deg,#0E9E6E,#10B981)" : "#C8D6CD", color: "#fff", fontWeight: 700, fontSize: 13, cursor: canGenerate ? "pointer" : "not-allowed" }}>
...
export default function ReportSection({
  ...
  // Documents still marked "Have to Ask" (unresolved). A Final Report requires
  // every document to be resolved — set to Available or Not Required.
  const getPendingDocs = React.useCallback(() => {
    const defaultDocs = buildDefaultDocs(clients, owners, hasBuilding, properties);
    const allLabels = [...defaultDocs.map(d => d.label), ...(customDocLabels || [])];
    return allLabels.filter(l => ((docChecks || {})[l] || "ask_client") === "ask_client");
  }, [clients, owners, hasBuilding, properties, customDocLabels, docChecks]);

  // Gate + dialog chain for a Final Report. Returns false if blocked.
  const startFinalFlow = React.useCallback(() => {
    const pending = getPendingDocs();
    if (pending.length) {
      showToast(`⚠ Complete Document Status first — ${pending.length} document(s) still marked "Have to Ask". Set each to Available or Not Required.`);
      return false;
    }
    ...
    return true;
  }, [getPendingDocs, showToast, collectState]);
  ...
  }, [reportType, fieldChargeReceived, doPreviewBill, doGenerate, startFinalFlow]);
  ...
  React.useEffect(() => {
    if (prevTypeRef.current !== reportType) {
      ...
      if (reportType === "bill") {
        ...
        // Switching to Final still enforces the document + payment gate.
        startFinalFlow();
      } else if (html !== null) {
        doGenerate();
      }
    }
  }, [reportType, html, doGenerate, doPreviewBill, startFinalFlow]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {balance !== null && reportType !== "bill" && (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8, padding: "10px 16px", borderRadius: 10, background: critical ? "#fff5f5" : low ? "#fffbea" : "#E8F5EC", border: `1.5px solid ${critical ? "#e74c3c" : low ? "#f39c12" : "#10B981"}` }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: critical ? "#c0392b" : low ? "#7a5c00" : "#0E9E6E" }}>
          ...
          <span style={{ fontSize: 12, color: "#7A8F87" }}>This {reportType} report costs <strong>{cost} credit{cost > 1 ? "s" : ""}</strong></span>
        </div>
      )}
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          {fullHtml && reportType === "bill" && (
            <button onClick={doPrintBill}
              style={{ padding: "10px 20px", background: "#D4AF37", color: "#fff", border: "none", borderRadius: "var(--radius)", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
              🖨 Print / Download Bill
            </button>
          )}
          {fullHtml && reportType !== "bill" && (
            <button onClick={handlePrint}
              style={{ padding: "10px 20px", background: "#0E9E6E", color: "#fff", border: "none", borderRadius: "var(--radius)", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
              🖨️ Print / Save as PDF
            </button>
          )}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
        {[
          ...
          <div key={type} onClick={() => setReportType(type)}
            style={{ padding: "14px 16px", borderRadius: 10, border: `2px solid ${reportType === type ? color : "#D6DCE5"}`, background: reportType === type ? bg : "#F8FAF9", cursor: "pointer", transition: "all 0.15s" }}>
            <div style={{ fontWeight: 700, fontSize: 14, color, marginBottom: 4 }}>{label}</div>
            <div style={{ fontSize: 12, color: "#7A8F87", lineHeight: 1.5 }}>{desc}</div>
          </div>
        ))}
      </div>
      ...
      {reportType === "preliminary" && fieldChargeReceived !== null && (
        <div style={{ background: "#fff", border: "1.5px solid #D6DCE5", borderRadius: 12, overflow: "hidden" }}>
          <div style={{ background: "linear-gradient(135deg,#064E3B,#0A6B50)", color: "#fff", padding: "10px 18px", fontWeight: 700, fontSize: 13, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ padding: "14px 18px", display: "flex", gap: 20, flexWrap: "wrap", fontSize: 13 }}>
            <div><span style={{ color: "#7A8F87" }}>Field Charge Received:</span> <strong style={{ color: fieldChargeReceived ? "#10B981" : "#e74c3c" }}>{fieldChargeReceived ? "Yes" : "No"}</strong></div>
            {fieldChargeReceived && fieldChargeAmount && <div><span style={{ color: "#7A8F87" }}>Amount:</span> <strong>{npr(fieldChargeAmount)}</strong></div>}
            {fieldChargeReceived && transportationCharge && <div><span style={{ color: "#7A8F87" }}>Transport:</span> <strong>{npr(transportationCharge)}</strong></div>}
          </div>
        </div>
      )}
      ...
      {reportType === "final" && (
        <div style={{ background: "#fff", border: "1.5px solid #D6DCE5", borderRadius: 12, overflow: "hidden" }}>
          <div style={{ background: "linear-gradient(135deg,#0E9E6E,#10B981)", color: "#fff", padding: "10px 18px", fontWeight: 700, fontSize: 13 }}>
            💰 Payment Received
          </div>
          <div style={{ padding: "16px 18px", display: "flex", alignItems: "flex-end", gap: 20, flexWrap: "wrap" }}>
            <div style={{ flex: "1 1 220px" }}>
              <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "#7A8F87", textTransform: "uppercase", marginBottom: 6 }}>
                Amount Received from Client (NPR)
              </label>
              <input
                type="number" min="0" placeholder="e.g. 15000"
                ...
                style={{ width: "100%", padding: "10px 12px", border: "1.5px solid #D6DCE5", borderRadius: 8, fontSize: 15, boxSizing: "border-box" }}
              />
              <div style={{ fontSize: 11, color: "#aaa", marginTop: 5 }}>Leave blank if not applicable. This appears on the final report.</div>
            </div>
            {amountReceived && Number(amountReceived) > 0 && (
              <div style={{ fontSize: 14, fontWeight: 700, color: "#0E9E6E", paddingBottom: 28 }}>
      ...
      {reportType === "bill" && (() => {
        ...
        const Row = ({ label, value, bold, total: isTotal, color, sub, highlight }) => (
          <tr style={{ background: isTotal ? "#E8F5EC" : highlight ? "#fff8e1" : sub ? "#F8FAF9" : "#fff", borderTop: isTotal ? "2px solid #064E3B" : "none" }}>
            ...
            <td style={{ padding: "7px 14px", textAlign: "right", fontWeight: bold||isTotal ? 800 : 600, fontSize: isTotal ? 15 : 13, color: color || (isTotal ? C2.navy : "#3D5A50") }}>{value}</td>
          </tr>
        );
        return (
          <div style={{ background: "#fff", border: "1.5px solid #D6DCE5", borderRadius: 12, overflow: "hidden" }}>
            <div style={{ background: "linear-gradient(135deg,#0E9E6E,#10B981)", color: "#fff", padding: "12px 18px", fontWeight: 700, fontSize: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ padding: "10px 18px", borderBottom: "1px solid #f0f0f0", background: "#F8FAF9", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              ...
              {[
                ...
                <label key={opt.value} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: billingSystem === opt.value ? 700 : 400, cursor: "pointer",
                  padding: "5px 12px", borderRadius: 20, border: `1.5px solid ${billingSystem === opt.value ? C2.navy : "#D6DCE5"}`,
            ...
            <div style={{ padding: "14px 18px", borderBottom: "1px solid #eee", display: "flex", gap: 12, flexWrap: "wrap" }}>
              <div style={{ flex: "1 1 180px" }}>
                ...
                <input type="text" value={billNo} onChange={e => setBillNo(e.target.value)} placeholder="e.g. 2081-082/001"
                  style={{ width: "100%", padding: "8px 10px", border: "1.5px solid #D6DCE5", borderRadius: 7, fontSize: 13, boxSizing: "border-box" }} />
              </div>
              ...
              {paymentMethods.length > 0 && (
                <div style={{ flex: "1 1 220px" }}>
                  ...
                  <select value={selectedPaymentMethodId} onChange={e => setSelectedPaymentMethodId(e.target.value)}
                    style={{ width: "100%", padding: "8px 10px", border: "1.5px solid #D6DCE5", borderRadius: 7, fontSize: 13, boxSizing: "border-box", background: "#fff" }}>
                  ...
                  {selectedPaymentMethodId && (() => {
                    ...
                    return pm ? (
                      <div style={{ marginTop: 5, fontSize: 11, color: C2.muted, lineHeight: 1.5 }}>
                        ...
                        {pm.qrCode && <span style={{ marginLeft: 8, color: "#10B981" }}>✓ QR attached</span>}
                      </div>
                    ) : null;
                  })()}
                </div>
            ...
            <div style={{ padding: "0 0 4px" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <tbody>
                  ...
                  <Row label="1. Field Visit Charge" value={npr(fieldVisit)} bold color="#0E9E6E" />
                  <Row label="2. Valuation Charge (NRB Schedule)" value={npr(valuationFee)} bold color="#0E9E6E" />
