...
function Legend({ minRate, maxRate }) {
  ...
  return (
    <div style={{
      ...
      fontFamily: "'Poppins', system-ui, -apple-system, 'Segoe UI', sans-serif", minWidth: 160,
    }}>
...
function StatsPanel({ points, filtered }) {
  ...
  return (
    <div style={{
      ...
      padding: "14px 18px", boxShadow: "0 4px 16px rgba(0,0,0,0.18)",
      fontFamily: "'Poppins', system-ui, -apple-system, 'Segoe UI', sans-serif", minWidth: 200,
    }}>
      <div style={{ fontWeight: 700, fontSize: 13, color: "#064E3B", marginBottom: 10 }}>📊 Map Statistics</div>
      {[
        ...
        <div key={label} style={{ display: "flex", justifyContent: "space-between", gap: 12, marginBottom: 5, fontSize: 12 }}>
          ...
          <span style={{ fontWeight: 600, color: "#064E3B" }}>{value}</span>
        </div>
      ))}
    </div>
  );
...
function popupHtml(p) {
  ...
  return `
    <div style="font-family:'Segoe UI',sans-serif;min-width:220px;max-width:280px">
      <div style="background:#064E3B;color:#fff;padding:8px 12px;margin:-12px -12px 10px;border-radius:6px 6px 0 0;font-weight:700;font-size:13px">
...
export default function ContourMap({ user, companies = [] }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: "'Poppins', system-ui, -apple-system, 'Segoe UI', sans-serif" }}>

      {/* ── Toolbar ── */}
      <div style={{
        background: "#064E3B", color: "#fff", padding: "10px 20px",
        display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap",
        borderBottom: "2px solid #0A6B50",
      }}>
        ...
        <button
          onClick={loadData}
          disabled={loading}
          style={{ padding: "5px 14px", background: "#D4AF37", color: "#fff", border: "none", borderRadius: 6, fontWeight: 700, fontSize: 12, cursor: "pointer", marginLeft: "auto" }}
        >
      ...
      <div style={{ flex: 1, position: "relative", minHeight: 0 }}>
        {!leafletReady && (
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", background: "#F3F6F5", zIndex: 10, fontSize: 15, color: "#666" }}>
            Loading map…
          </div>
        )}
        {loading && (
          <div style={{ position: "absolute", top: 16, left: "50%", transform: "translateX(-50%)", zIndex: 1000, background: "rgba(6, 78, 59,0.85)", color: "#fff", padding: "8px 20px", borderRadius: 20, fontSize: 13, fontWeight: 600 }}>
            ⏳ Loading plot data…
          </div>
        )}
        ...
        {!loading && leafletReady && filtered.length === 0 && points.length === 0 && (
          <div style={{
            ...
            <div style={{ fontWeight: 700, fontSize: 16, color: "#064E3B", marginBottom: 6 }}>No plot data found</div>
