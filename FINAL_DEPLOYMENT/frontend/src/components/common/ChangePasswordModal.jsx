...
const overlay = {
  ...
  zIndex: 9999, fontFamily: "'Poppins', system-ui, -apple-system, 'Segoe UI', sans-serif",
};
const card = {
...
function PasswordStrength({ password }) {
  ...
  return (
    <ul style={{ margin: "6px 0 0", padding: 0, listStyle: "none" }}>
      {checks.map(({ label, ok }) => (
        <li key={label} style={{ fontSize: 11, color: ok ? "#10B981" : "#aaa", marginBottom: 2 }}>
          {ok ? "✓" : "○"} {label}
        </li>
      ))}
    </ul>
...
export default function ChangePasswordModal({ user, onDone }) {
  ...
  return (
    <div style={overlay}>
      <div style={card}>
        <h2 style={{ margin: "0 0 6px", color: "#064E3B", fontSize: 20 }}>Change Password</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
            ...
            <button
              style={{ ...btn, background: loading ? "#aaa" : "#10B981", color: "#fff" }}
              type="submit"
              disabled={loading}
            >
