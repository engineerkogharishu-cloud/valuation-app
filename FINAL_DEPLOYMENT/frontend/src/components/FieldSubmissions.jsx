...
export default function FieldSubmissions({ onUseData, user }) {
  ...
  return (
    <div>
      ...
      {isAdmin && (
        <div style={{ background: "linear-gradient(135deg,#064E3B,#0A6B50)", borderRadius: 14, padding: "20px 22px", marginBottom: 24, color: "#fff", boxShadow: "0 4px 20px rgba(0,0,0,0.18)" }}>
          {/* Header */}
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
            ...
            <button onClick={() => setShowLinkForm(f => !f)}
              style={{ padding: "8px 16px", background: showLinkForm ? "rgba(255,255,255,0.1)" : "#10B981", border: "none", borderRadius: 8, color: "#fff", cursor: "pointer", fontSize: 13, fontWeight: 700 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
                <div style={{ flex: 1, minWidth: 140 }}>
                  ...
                  <input value={newLinkLabel} onChange={e => setNewLinkLabel(e.target.value)}
                    ...
                    style={{ width: "100%", padding: "8px 10px", borderRadius: 7, border: "1px solid rgba(255,255,255,0.2)", background: "#fff", color: "#064E3B", fontSize: 13, boxSizing: "border-box", outline: "none" }} />
                </div>
                <div>
                  ...
                  <div style={{ display: "flex", gap: 0, borderRadius: 7, overflow: "hidden", border: "1px solid rgba(255,255,255,0.2)" }}>
                    {["permanent","temporary"].map(t => (
                      <button key={t} onClick={() => setNewLinkType(t)}
                        style={{ padding: "8px 14px", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 700, background: newLinkType === t ? "#10B981" : "rgba(255,255,255,0.08)", color: "#fff" }}>
                ...
                <button onClick={createLink} disabled={genLoading}
                  style={{ padding: "8px 18px", background: "#34D399", border: "none", borderRadius: 7, color: "#fff", cursor: "pointer", fontSize: 13, fontWeight: 700, whiteSpace: "nowrap" }}>
            ...
            return (
              <div key={lnk.id} style={{ background: expired ? "rgba(180,0,0,0.18)" : "rgba(0,0,0,0.22)", borderRadius: 10, padding: "10px 14px", marginBottom: 8, opacity: expired ? 0.7 : 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  ...
                  {/* Short URL */}
                  <div style={{ fontFamily: "monospace", fontSize: 13, color: "#E0C468", background: "rgba(0,0,0,0.3)", borderRadius: 6, padding: "4px 10px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 220 }}>
                    {url.replace(/^https?:\/\//, "")}
                  </div>
                  {/* Actions */}
                  <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
                    <button onClick={() => copyUrl(lnk.short_code)}
                      style={{ padding: "5px 12px", background: isCopied ? "#34D399" : "#10B981", border: "none", borderRadius: 6, color: "#fff", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>
                      {isCopied ? "✓" : "Copy"}
                    </button>
                    {navigator.share && (
                ...
                {isQr && (
                  <div style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 14 }}>
                    ...
                    <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", lineHeight: 1.6 }}>
                      ...
                      <span style={{ fontFamily: "monospace", color: "#E0C468", fontSize: 11 }}>{url}</span>
                    </div>
                  </div>
                )}
              </div>
      ...
      {(() => {
        ...
        const tabs = [
          ...
          { key: "pulled",   label: "Used",     count: counts.pulled,   color: "#0E9E6E", bg: "#E8F5EC" },
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
            {tabs.map(t => (
              <button key={t.key} onClick={() => setActiveTab(t.key)} style={{
                ...
                background: activeTab === t.key ? t.color : "#F3F6F5",
            ...
            {activeTab === "rejected" && isAdmin && submissions.filter(s => s.status === "rejected").length > 0 && (
              <button onClick={exportRejected} style={{ marginLeft: "auto", padding: "7px 14px", background: "#064E3B", color: "#fff", border: "none", borderRadius: 20, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                ⬇ Export CSV
              </button>
            )}
          </div>
        ...
        <div style={S.overlay}>
          <div style={S.modal}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
              <h3 style={{ margin: 0, color: "#064E3B" }}>Field Submission #{viewId}</h3>
            ...
            <div style={{ ...S.modal, maxWidth: 500 }}>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <input
                  ...
                  placeholder="e.g. 523"
                  style={{ width: "100%", padding: "9px 12px", border: "1.5px solid #C8D6CD", borderRadius: 8, fontSize: 14, boxSizing: "border-box", fontFamily: "inherit", background: rejectExtra.plotNo ? "#F3F6F5" : "#fff", fontWeight: rejectExtra.plotNo ? 600 : 400 }}
                />
              </div>
...
const S = {
  linkBox: { background: "#F8FAF9", border: "1px solid #D6DCE5", borderRadius: 10, padding: "18px 20px", marginBottom: 28 },
  btnGenerate: { padding: "10px 20px", background: "#064E3B", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 600, fontSize: 14 },
  linkInput: { flex: 1, minWidth: 200, padding: "9px 12px", border: "1px solid #ccc", borderRadius: 6, fontSize: 13, background: "#fff" },
  btnCopy: { padding: "9px 16px", background: "#10B981", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: 600, fontSize: 13 },
  sectionTitle: { fontSize: 20, fontWeight: 700, color: "#064E3B", margin: 0 },
  btnRefresh: { padding: "7px 14px", background: "#F3F6F5", border: "1px solid #ddd", borderRadius: 6, cursor: "pointer", fontSize: 13 },
  btnImport:  { padding: "7px 16px", background: "#064E3B", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer", fontSize: 13, fontWeight: 600 },
  ...
  th: { background: "#064E3B", color: "#fff", padding: "12px 14px", textAlign: "left", fontWeight: 600, whiteSpace: "nowrap" },
  ...
  badgePulled:  { background: "#E8F5EC", color: "#0E9E6E" },
  btnView: { background: "#064E3B", color: "#fff", border: "none", borderRadius: 5, padding: "4px 10px", cursor: "pointer", fontSize: 12, marginRight: 5 },
  btnUse:  { background: "#D4AF37", color: "#fff", border: "none", borderRadius: 5, padding: "4px 10px", cursor: "pointer", fontSize: 12, marginRight: 5, fontWeight: 600 },
  ...
  closeBtn: { background: "#F3F6F5", border: "none", borderRadius: 6, width: 30, height: 30, cursor: "pointer", fontSize: 14, color: "#666" },
  btnUseModal: { padding: "9px 20px", background: "#D4AF37", color: "#fff", border: "none", borderRadius: 7, cursor: "pointer", fontWeight: 700, fontSize: 14 },
