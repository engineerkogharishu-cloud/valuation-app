...
function RateDetailPanel({ point, onClose }) {
  ...
  const C = { navy: "#064E3B", border: "#D6DCE5", muted: "#7A8F87" };
  const rows = [
  ...
  return createPortal(
    <div style={{ position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.5)", backdropFilter: "blur(2px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 99999, padding: 16 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 420, boxShadow: "0 24px 64px rgba(0,0,0,0.3)", overflow: "hidden", fontFamily: "'Poppins', system-ui, -apple-system, 'Poppins', system-ui, -apple-system, 'Segoe UI', sans-serif" }}>
        <div style={{ background: point.isOwn ? "linear-gradient(135deg,#10B981,#0E9E6E)" : "linear-gradient(135deg,#064E3B,#0A6B50)", padding: "18px 22px", color: "#fff", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
...
function SessionDialog({ access, onStarted, onClose }) {
  ...
  return createPortal(
    <div style={{ position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.6)", backdropFilter: "blur(3px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 99999, padding: 16 }}>
      <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 480, boxShadow: "0 24px 64px rgba(0,0,0,0.35)", overflow: "hidden", fontFamily: "'Poppins', system-ui, -apple-system, 'Poppins', system-ui, -apple-system, 'Segoe UI', sans-serif" }}>
        <div style={{ background: "linear-gradient(135deg,#064E3B,#0A6B50)", padding: "22px 26px", color: "#fff" }}>
        ...
        <div style={{ padding: "22px 26px" }}>
          {access.free && (
            <div style={{ background: "#E8F5EC", border: "1.5px solid #10B981", borderRadius: 10, padding: "10px 14px", fontSize: 13, color: "#0E9E6E", fontWeight: 600, marginBottom: 18 }}>
              ✅ Free Access Enabled by Super Admin — no credits will be deducted.
            </div>
          )}
          {!access.free && (
            <div style={{ background: "#F3F6F5", borderRadius: 10, padding: "10px 14px", fontSize: 13, marginBottom: 18, display: "flex", justifyContent: "space-between" }}>
              <span style={{ color: "#7A8F87" }}>Your Credit Balance</span>
              <span style={{ fontWeight: 700, color: "#064E3B" }}>🪙 {access.balance}</span>
            </div>
          )}
          <div style={{ fontSize: 12, fontWeight: 700, color: "#7A8F87", textTransform: "uppercase", marginBottom: 10 }}>Select Usage Duration</div>
            ...
            {durations.map(({ minutes, credits }) => {
              ...
              return (
                <div key={minutes} onClick={() => setSelected(minutes)}
                  style={{ padding: "16px 14px", border: `2px solid ${active ? "#10B981" : "#D6DCE5"}`, borderRadius: 12, background: active ? "#E8F5EC" : "#F8FAF9", cursor: affordable ? "pointer" : "not-allowed", textAlign: "center", opacity: affordable ? 1 : 0.5, transition: "all 0.15s" }}>
                  <div style={{ fontWeight: 700, fontSize: 20, color: active ? "#10B981" : "#064E3B" }}>⏱ {minutes} min</div>
                  {access.free
                    ? <div style={{ fontSize: 12, marginTop: 4, color: "#10B981", fontWeight: 600 }}>Free</div>
                    : <div style={{ fontSize: 14, marginTop: 6, color: active ? "#10B981" : "#7A8F87", fontWeight: 700 }}>🪙 {credits} Credits</div>}
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={onClose} disabled={loading} style={{ flex: 1, padding: "11px", border: "1.5px solid #D6DCE5", borderRadius: 9, background: "#fff", color: "#3D5A50", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>Cancel</button>
            <button onClick={handleStart} disabled={loading || !canAfford}
              style={{ flex: 2, padding: "11px", border: "none", borderRadius: 9, background: canAfford ? "linear-gradient(135deg,#10B981,#0E9E6E)" : "#ccc", color: "#fff", fontWeight: 700, fontSize: 13, cursor: canAfford && !loading ? "pointer" : "not-allowed", opacity: loading ? 0.7 : 1 }}>
...
function RateMap({ points, rejectedPoints = [] }) {
  ...
  useEffect(() => {
    ...
    for (const p of points) {
      ...
      const icon = L.divIcon({
        className: "",
        html: `<div style="width:${p.isOwn ? 18 : 14}px;height:${p.isOwn ? 18 : 14}px;border-radius:50%;background:${color};border:${p.isOwn ? "2.5px solid #fff" : "2px solid rgba(255,255,255,0.85)"};box-shadow:0 2px 6px rgba(0,0,0,0.4);cursor:pointer;${p.isOwn ? "box-shadow:0 0 0 3px rgba(16, 185, 129,0.35),0 2px 6px rgba(0,0,0,0.4);" : ""}"></div>`,
  ...
  return (
    <div style={{ position: "relative", flex: 1, minHeight: 480, borderRadius: 12, overflow: "hidden", border: "1px solid #D6DCE5" }}>
      ...
      <div style={{ position: "absolute", top: 14, right: 14, zIndex: 1000, background: "rgba(255,255,255,0.97)", borderRadius: 10, padding: "12px 16px", boxShadow: "0 4px 16px rgba(0,0,0,0.18)", fontSize: 12, minWidth: 190 }}>
        <div style={{ fontWeight: 700, fontSize: 13, color: "#064E3B", marginBottom: 8 }}>📊 Map Statistics</div>
        {[
        ...
        <div style={{ borderTop: "1px solid #eee", marginTop: 8, paddingTop: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 4 }}>
            <div style={{ width: 14, height: 14, borderRadius: "50%", background: "#10B981", border: "2px solid #fff", boxShadow: "0 0 0 2px rgba(16, 185, 129,0.35)", flexShrink: 0 }} />
      ...
      {points.length > 0 && !selected && !selectedRej && (
        <div style={{ position: "absolute", bottom: 14, left: "50%", transform: "translateX(-50%)", zIndex: 1000, background: "rgba(6, 78, 59,0.85)", color: "#fff", borderRadius: 20, padding: "6px 18px", fontSize: 12, fontWeight: 600, pointerEvents: "none", whiteSpace: "nowrap" }}>
      ...
      {selectedRej && createPortal(
        <div style={{ position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.5)", backdropFilter: "blur(2px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 99999, padding: 16 }} onClick={() => setSelectedRej(null)}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 420, boxShadow: "0 24px 64px rgba(0,0,0,0.3)", overflow: "hidden", fontFamily: "'Poppins', system-ui, -apple-system, 'Poppins', system-ui, -apple-system, 'Segoe UI', sans-serif" }}>
            {/* Red header */}
            ...
            <div style={{ padding: "16px 22px 20px" }}>
              {(() => {
                ...
                return [
                  ...
                  <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "8px 0", borderBottom: "1px solid #D6DCE5" }}>
                    <span style={{ fontSize: 13, color: "#7A8F87", fontWeight: 600, minWidth: 130 }}>{label}</span>
                    <span style={{ fontSize: 13, fontWeight: 700, color: "#064E3B", textAlign: "right", maxWidth: 230, wordBreak: "break-word" }}>{value}</span>
                  </div>
                ));
              })()}
...
function RejectedPanel({ reports }) {
  const C = { navy: "#064E3B", border: "#D6DCE5", muted: "#7A8F87", danger: "#c0392b" };
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 12, border: `1.5px solid #e74c3c`, overflow: "hidden" }}>
      ...
      {viewing && (() => {
        ...
        return createPortal(
          <div style={{ position: "fixed", inset: 0, background: "rgba(6, 78, 59,0.55)", backdropFilter: "blur(2px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, padding: 16 }} onClick={() => setViewing(null)}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16, height: "calc(100vh - 200px)", minHeight: 620 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          {/* Status badges */}
          {access?.free ? (
            <span style={{ background: "#E8F5EC", color: "#0E9E6E", border: "1.5px solid #10B981", borderRadius: 20, padding: "5px 14px", fontSize: 12, fontWeight: 700 }}>✅ Free Access</span>
          ) : isTrialActive ? (
            ...
            <span style={{ background: countdown < 60000 ? "#fff5f5" : "#E8F5EC", color: countdown < 60000 ? "#e74c3c" : "#10B981", border: `1.5px solid ${countdown < 60000 ? "#e74c3c" : "#10B981"}`, borderRadius: 20, padding: "5px 14px", fontSize: 13, fontWeight: 700, fontVariantNumeric: "tabular-nums" }}>
              ⏱ {fmtCountdown(countdown)} remaining
            </span>
          ) : null}
          {/* Action buttons */}
          {!access?.free && (!hasAccess || isTrialActive) && (
            <button onClick={() => setShowDialog(true)} style={{ padding: "9px 20px", background: "linear-gradient(135deg,#10B981,#0E9E6E)", color: "#fff", border: "none", borderRadius: 9, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
              🗺️ View on Map
            </button>
          )}
          {hasAccess && (
            <button onClick={() => { setLoadingPts(true); api.getRateMapPoints().then(d => setPoints(d.points || [])).catch(() => {}).finally(() => setLoadingPts(false)); }}
              disabled={loadingPts} style={{ padding: "8px 16px", background: "#F3F6F5", border: "none", borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: "pointer" }}>
              ↺ Refresh
            </button>
          )}
        </div>
      ...
      {!access?.free && (!hasAccess || isTrialActive) && (
        <div style={{ display: "grid", gridTemplateColumns: `repeat(${durations.length}, 1fr)`, gap: 14 }}>
          {durations.map(({ minutes, credits }) => (
            <div key={minutes} onClick={() => setShowDialog(true)} style={{ padding: "18px 20px", borderRadius: 14, border: "2px solid #D6DCE5", background: "#F8FAF9", textAlign: "center", cursor: "pointer", transition: "border-color 0.15s" }}
              onMouseEnter={e => e.currentTarget.style.borderColor = "#10B981"}
              onMouseLeave={e => e.currentTarget.style.borderColor = "#D6DCE5"}>
              <div style={{ fontSize: 28, marginBottom: 6 }}>⏱</div>
              <div style={{ fontWeight: 700, fontSize: 16, color: C.navy }}>Use for {minutes} Minutes</div>
              <div style={{ fontSize: 22, fontWeight: 800, color: "#10B981", margin: "8px 0" }}>🪙 {credits} Credits</div>
      ...
      {hasAccess ? (
        loadingPts ? (
          <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", background: "#F8FAF9", borderRadius: 14, border: "2px dashed #D6DCE5", color: C.muted, fontSize: 15, fontWeight: 600 }}>
            ⏳ Loading rate points…
          </div>
        ) : (
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "#F8FAF9", borderRadius: 14, border: "2px dashed #D6DCE5", color: C.muted, gap: 12 }}>
          ...
          <button onClick={() => setShowDialog(true)} style={{ marginTop: 8, padding: "11px 28px", background: "linear-gradient(135deg,#10B981,#0E9E6E)", color: "#fff", border: "none", borderRadius: 10, fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
            🗺️ View on Map
          </button>
        </div>
      )}
