...
// ── CORS ──────────────────────────────────────────────────────────────────────
// The public health probe is the one endpoint that must answer a request with
// no Origin header: platform health checks (Railway, Fly.io) and uptime
// monitors are not browsers and never send one. Everything else still requires
// a browser-supplied Origin in production.
const NO_ORIGIN_ALLOWED = new Set(["/api/ping"]);

app.use(cors((req, done) => {
  done(null, {
    origin: (origin, cb) => {
      if (process.env.NODE_ENV === "production" && !origin && !NO_ORIGIN_ALLOWED.has(req.path)) {
...
// ── Body parsers ─────────────────────────────────────────────────────────────
// A few endpoints (report save, field submissions) legitimately receive large
// base64 payloads and declare their own express.json() with a bigger limit.
// body-parser skips a request whose body is already parsed, so a single global
// 12 MB parser would silently cap those endpoints at 12 MB. Pick the limit by
// path here so every request still gets a parser and the large endpoints get
// the limit their route actually declares.
const jsonDefault = express.json({ limit: "12mb" });
const jsonLarge   = express.json({ limit: "50mb" });
const LARGE_BODY_RE = /^\/api\/(reports|field\/submit|field\/submissions\/import)/;

app.use((req, res, next) =>
  (LARGE_BODY_RE.test(req.path) ? jsonLarge : jsonDefault)(req, res, next)
);
...
async function initDb() {
  ...
  const migrations = [
    ...
    // User account activation (admins can pause/resume users)
...
function auth(roles = []) {
  return async (req, res, next) => {
    ...
    try {
      ...
      // A user paused mid-session is locked out on their next request.
      if (decoded.role === "user") {
        const acct = await dbGet("SELECT is_active FROM users WHERE id=?", [decoded.userId]);
        if (acct && acct.is_active === 0)
          return res.status(403).json({ error: "This account has been paused. Please contact your administrator." });
      }
      req.user = decoded;
      next();
    } catch {
...
app.post("/api/auth/login", loginLimiter, async (req, res) => {
  try {
    ...
    const user = await dbGet(
      `SELECT id, company_code, username, email, role, must_change_password, is_active, password_hash
    ...
    // Paused accounts cannot sign in (only 'user' role can be paused).
    if (user.is_active === 0) {
      logEvent("LOGIN_PAUSED", req, `company=${cc} username=${usr}`);
      return res.status(403).json({ error: "This account has been paused. Please contact your administrator." });
    }
...
app.get("/api/users", auth(["super_user", "admin"]), async (req, res) => {
  try {
    ...
    if (req.user.role === "super_user") {
      users = await dbAll(
        "SELECT id, company_code, username, email, role, must_change_password, is_active, created_at FROM users ORDER BY company_code, username"
      );
    } else {
      users = await dbAll(
        "SELECT id, company_code, username, email, role, must_change_password, is_active, created_at FROM users WHERE company_code=? AND role='user' ORDER BY username",
        [req.user.companyCode]
      );
    }
    res.json(users);
...
// Pause / resume a user account. Paused users cannot sign in.
app.patch("/api/users/:id/active", auth(["super_user", "admin"]), async (req, res) => {
  try {
    const user = await dbGet("SELECT * FROM users WHERE id=?", [req.params.id]);
    if (!user) return res.status(404).json({ error: "Not found" });
    if (req.user.role === "admin" && user.company_code !== req.user.companyCode)
      return res.status(403).json({ error: "Forbidden" });
    if (user.id === req.user.userId) return res.status(400).json({ error: "You cannot pause your own account" });
    if (user.role !== "user") return res.status(400).json({ error: "Only regular users can be paused" });

    const active = req.body.active ? 1 : 0;
    await dbRun("UPDATE users SET is_active=?, updated_at=CURRENT_TIMESTAMP WHERE id=?", [active, req.params.id]);
    logEvent(active ? "USER_RESUMED" : "USER_PAUSED", req, `target_id=${req.params.id} username=${user.username}`);
    res.json({ message: active ? "User resumed" : "User paused", is_active: active });
  } catch (err) {
    handleError(res, err, "PATCH /api/users/:id/active");
  }
});
...
const FROM_NAME  = process.env.EMAIL_FROM_NAME  || "Mulyanka";
...
async function sendRegistrationReceivedEmail(toEmail, contactName, companyName) {
  ...
  try {
    await mailer.sendMail({
      from: emailFrom(),
      to: toEmail,
      subject: "Registration Request Received — Mulyanka",
      html: `
        <div style="font-family:'Poppins','Segoe UI',Arial,sans-serif;max-width:580px;margin:0 auto;background:#fff;border-radius:14px;overflow:hidden;border:1px solid #D6DCE5;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
          <!-- Header -->
          <div style="background:linear-gradient(135deg,#064E3B 0%,#0A6B50 100%);padding:32px 36px;color:#fff">
            <div style="font-size:36px;margin-bottom:10px">📋</div>
            <h1 style="margin:0;font-size:22px;font-weight:800;letter-spacing:-0.3px">Registration Request Received</h1>
            <p style="margin:6px 0 0;opacity:0.65;font-size:13px">Mulyanka — One Degree Consultant Pvt. Ltd.</p>
          </div>
          <!-- Body -->
          <div style="padding:32px 36px">
            <p style="font-size:15px;color:#064E3B;margin:0 0 14px;line-height:1.6">
              Dear <strong>${contactName}</strong>,
            </p>
            <p style="font-size:14px;color:#3D5A50;line-height:1.7;margin:0 0 20px">
              Thank you for registering <strong>${companyName}</strong> on Mulyanka. We have received your request and our team is currently reviewing it.
            </p>
            <!-- Status box -->
            <div style="background:#E8F5EC;border-radius:10px;padding:18px 22px;margin:0 0 22px;border-left:4px solid #10B981">
              <div style="font-size:11px;font-weight:800;color:#10B981;text-transform:uppercase;letter-spacing:0.6px;margin-bottom:6px">What happens next?</div>
              <ol style="margin:0;padding-left:18px;font-size:13px;color:#3D5A50;line-height:1.9">
            ...
            <p style="font-size:13px;color:#7A8F87;line-height:1.6;margin:0 0 24px">
              This process typically takes <strong>less than 24 hours</strong>. If you have any questions, feel free to reply to this email.
            </p>
            <hr style="border:none;border-top:1px solid #D6DCE5;margin:0 0 20px">
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td style="font-size:12px;color:#a0aec0">
                  <strong style="color:#3D5A50">One Degree Consultant Pvt. Ltd.</strong><br>
                  Property Valuation Management System<br>
                  <a href="mailto:onedegreeconsultant@gmail.com" style="color:#10B981;text-decoration:none">onedegreeconsultant@gmail.com</a>
                  &nbsp;·&nbsp; 9841357433
                </td>
  ...
  try {
    await mailer.sendMail({
      ...
      html: `
        <div style="font-family:'Poppins','Segoe UI',Arial,sans-serif;max-width:580px;margin:0 auto;background:#fff;border-radius:14px;overflow:hidden;border:1px solid #D6DCE5;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
          <!-- Header -->
          <div style="background:linear-gradient(135deg,#1a5c3a 0%,#10B981 100%);padding:32px 36px;color:#fff">
            <div style="font-size:36px;margin-bottom:10px">🎉</div>
            <h1 style="margin:0;font-size:22px;font-weight:800;letter-spacing:-0.3px">Your Account is Ready!</h1>
            <p style="margin:6px 0 0;opacity:0.75;font-size:13px">Mulyanka — One Degree Consultant Pvt. Ltd.</p>
          </div>
          <!-- Body -->
          <div style="padding:32px 36px">
            <p style="font-size:15px;color:#064E3B;margin:0 0 14px;line-height:1.6">
            ...
            <p style="font-size:14px;color:#3D5A50;line-height:1.7;margin:0 0 22px">
              Great news! Your registration for <strong>${companyName}</strong> has been <strong style="color:#10B981">approved</strong>. Your company account has been created on Mulyanka and is ready to use.
            </p>

            <!-- Login credentials box -->
            <div style="background:linear-gradient(135deg,#E8F5EC,#dcfce7);border-radius:12px;padding:22px 26px;margin:0 0 24px;border:1.5px solid #86efac">
              <div style="font-size:11px;font-weight:800;color:#0E9E6E;text-transform:uppercase;letter-spacing:0.6px;margin-bottom:14px">🔑 Your Login Credentials</div>
              <table cellpadding="0" cellspacing="0" style="width:100%">
                <tr>
                  <td style="font-size:12px;font-weight:700;color:#0E9E6E;text-transform:uppercase;letter-spacing:0.4px;padding:5px 0;width:40%">Company Code</td>
                  <td style="font-size:15px;font-weight:800;color:#064E3B;font-family:monospace;padding:5px 0">${companyCode}</td>
                </tr>
                <tr>
                  <td style="font-size:12px;font-weight:700;color:#0E9E6E;text-transform:uppercase;letter-spacing:0.4px;padding:5px 0">Username</td>
                  <td style="font-size:15px;font-weight:800;color:#064E3B;font-family:monospace;padding:5px 0">${adminUsername}</td>
                </tr>
                <tr>
                  <td style="font-size:12px;font-weight:700;color:#0E9E6E;text-transform:uppercase;letter-spacing:0.4px;padding:5px 0">Password</td>
                  <td style="font-size:13px;color:#3D5A50;padding:5px 0">
            ...
            <!-- Steps -->
            <div style="background:#F8FAF9;border-radius:10px;padding:18px 22px;margin:0 0 22px;border-left:4px solid #064E3B">
              <div style="font-size:11px;font-weight:800;color:#064E3B;text-transform:uppercase;letter-spacing:0.6px;margin-bottom:10px">Getting started</div>
              <ol style="margin:0;padding-left:18px;font-size:13px;color:#3D5A50;line-height:2">
                <li>Visit the Mulyanka login page</li>
...
async function sendFeedbackApprovalEmail(toEmail, toName, feedbackMessage) {
  ...
  try {
    await mailer.sendMail({
      ...
      html: `
        <div style="font-family:'Poppins','Segoe UI',Arial,sans-serif;max-width:560px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #D6DCE5">
          ...
          <div style="padding:28px 32px">
            <p style="font-size:15px;color:#3D5A50;line-height:1.6">Dear <strong>${toName}</strong>,</p>
            ...
            <div style="background:#f4f6fa;border-radius:10px;padding:16px 20px;margin:20px 0;border-left:4px solid #10B981">
              <div style="font-size:11px;font-weight:700;color:#7f8c8d;text-transform:uppercase;margin-bottom:6px">Your Feedback</div>
              <p style="margin:0;font-size:13px;color:#3D5A50;font-style:italic">"${feedbackMessage}"</p>
            </div>
            <div style="background:linear-gradient(135deg,#10B981,#0E9E6E);border-radius:10px;padding:16px 22px;display:flex;align-items:center;gap:14px;margin:20px 0">
            ...
            <hr style="border:none;border-top:1px solid #D6DCE5;margin:24px 0">
...
async function sendCreditsAssignedEmail(toEmail, contactName, companyName, creditsAdded, newBalance, note) {
  ...
  try {
    await mailer.sendMail({
      ...
      html: `
        <div style="font-family:'Poppins','Segoe UI',Arial,sans-serif;max-width:560px;margin:0 auto;background:#fff;border-radius:14px;overflow:hidden;border:1px solid #D6DCE5;box-shadow:0 4px 24px rgba(0,0,0,0.07)">
          <div style="background:linear-gradient(135deg,#1a5c3a 0%,#10B981 100%);padding:30px 34px;color:#fff">
            <div style="font-size:36px;margin-bottom:8px">🪙</div>
            <h1 style="margin:0;font-size:21px;font-weight:800">Credits Added to Your Account</h1>
            <p style="margin:6px 0 0;opacity:0.75;font-size:13px">Mulyanka — One Degree Consultant Pvt. Ltd.</p>
          </div>
          <div style="padding:30px 34px">
            <p style="font-size:15px;color:#064E3B;margin:0 0 14px;line-height:1.6">Dear <strong>${contactName || companyName}</strong>,</p>
            <p style="font-size:14px;color:#3D5A50;line-height:1.7;margin:0 0 22px">
            ...
            <div style="background:linear-gradient(135deg,#E8F5EC,#dcfce7);border-radius:12px;padding:22px 26px;margin:0 0 22px;border:1.5px solid #86efac;display:flex;justify-content:space-between;align-items:center">
              <div>
                <div style="font-size:11px;font-weight:800;color:#0E9E6E;text-transform:uppercase;letter-spacing:0.6px;margin-bottom:6px">Credits Added</div>
                <div style="font-size:32px;font-weight:900;color:#15803d">+${creditsAdded}</div>
              </div>
              <div style="text-align:right">
                <div style="font-size:11px;font-weight:800;color:#0E9E6E;text-transform:uppercase;letter-spacing:0.6px;margin-bottom:6px">New Balance</div>
                <div style="font-size:28px;font-weight:900;color:#064E3B">${newBalance}</div>
              </div>
            </div>
            ${note ? `<div style="background:#F8FAF9;border-radius:8px;padding:14px 18px;margin:0 0 20px;border-left:4px solid #10B981;font-size:13px;color:#3D5A50"><strong>Note from admin:</strong> ${note}</div>` : ""}
            <p style="font-size:13px;color:#7A8F87;line-height:1.6;margin:0 0 24px">
            ...
            <p style="font-size:12px;color:#a0aec0;margin:0">
              <strong style="color:#3D5A50">One Degree Consultant Pvt. Ltd.</strong><br>
              <a href="mailto:onedegreeconsultant@gmail.com" style="color:#10B981;text-decoration:none">onedegreeconsultant@gmail.com</a> &nbsp;·&nbsp; 9841357433
            </p>
          </div>
        </div>`,
    });
...
app.post("/api/admin/email/test", auth(["super_user"]), express.json(), async (req, res) => {
  try {
    ...
    await mailer.sendMail({
      from: emailFrom(),
      to,
      subject: "✅ Test Email — Mulyanka",
      html: `<div style="font-family:'Poppins','Segoe UI',Arial,sans-serif;max-width:500px;margin:0 auto;padding:32px;background:#F8FAF9;border-radius:12px;border:1px solid #D6DCE5">
        <div style="font-size:40px;text-align:center;margin-bottom:16px">✅</div>
        <h2 style="text-align:center;color:#064E3B;margin:0 0 12px">Email System Working!</h2>
        <p style="color:#3D5A50;text-align:center;font-size:14px;line-height:1.7">
          This is a test email from <strong>Mulyanka</strong>, by One Degree Consultant Pvt. Ltd.<br/>
          If you received this, your email configuration is correct.
        </p>
        <hr style="border:none;border-top:1px solid #D6DCE5;margin:20px 0"/>
...
// Anything thrown or passed to next(err) lands here. Never leak a stack trace
// or an internal message to the client; the full error goes to the server log.
// eslint-disable-next-line no-unused-vars
app.use((err, req, res, next) => {
  if (res.headersSent) return next(err);

  // Rejected by the CORS origin callback.
  if (err && /^CORS/.test(err.message || "")) {
    console.warn(`[CORS] blocked origin=${req.headers.origin || "(none)"} path=${req.path}`);
...
if (require.main === module) {
  ...
  // Keep the process alive on an unexpected async failure; log it loudly.
  ...
  // Graceful shutdown so in-flight requests finish on a platform redeploy.
